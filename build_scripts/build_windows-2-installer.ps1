...
& "$NPM_PATH/electron-builder.ps1" build --win --x64 --config.productName="Chik" --dir --config ../../../build_scripts/electron-builder.json
if ($LASTEXITCODE -ne 0) {
...
$installerPath = ".\dist\ChikSetup-$packageVersion.exe"
...
Copy-Item $installerPath -Destination "$env:GITHUB_WORKSPACE\chik-blockchain-gui\release-builds\windows-installer"
