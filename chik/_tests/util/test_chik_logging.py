...
from chik.util.chik_logging import JournalSocketHandler, initialize_logging
...
def test_systemd_logging_enabled_installed(tmp_path: Path) -> None:
    ...
    with patch("chik.util.chik_logging.JournalSocketHandler", return_value=mock_handler_instance) as mock_handler_class:
        with patch("chik.util.chik_logging.get_file_log_handler", return_value=MagicMock(level=logging.INFO)):
            initialize_logging("test_service", logging_config, tmp_path)

    mock_handler_class.assert_called_once_with(identifier="chik.test_service")
...
def test_systemd_logging_enabled_not_installed(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with patch("chik.util.chik_logging.JournalSocketHandler", side_effect=OSError("missing socket")):
        with patch("chik.util.chik_logging.get_file_log_handler", return_value=MagicMock(level=logging.INFO)):
...
def test_systemd_logging_disabled(tmp_path: Path) -> None:
    ...
    with patch("chik.util.chik_logging.JournalSocketHandler") as mock_handler_class:
        with patch("chik.util.chik_logging.get_file_log_handler", return_value=MagicMock(level=logging.INFO)):
            initialize_logging("test_service", logging_config, tmp_path)
...
def test_journal_socket_handler_emits_native_entry() -> None:
    ...
    with patch("chik.util.chik_logging.socket.AF_UNIX", 1, create=True):
        with patch("chik.util.chik_logging.socket.socket", return_value=mock_socket):
            handler = JournalSocketHandler(identifier="chik.test_service")
    ...
    assert b"PRIORITY=3\n" in payload
    assert b"SYSLOG_IDENTIFIER=chik.test_service\n" in payload
