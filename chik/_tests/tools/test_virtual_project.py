...
from chik.util.virtual_project_analysis import (
    Annotation,
    ChikFile,
    Config,
    DirectoryParameters,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("# Some comment\n# Some other comment", "chik-blockchain", False),
    ],
)
...
def test_parse_with_annotation(create_test_file: Callable[[str, str], Path]) -> None:
    ...
    parsed_file = ChikFile.parse(test_file)
...
def test_parse_nonexistent_file() -> None:
    ...
    with pytest.raises(FileNotFoundError):
        ChikFile.parse(Path("/path/to/nonexistent/file.txt"))
...
@pytest.fixture
def chik_package_structure(tmp_path: Path) -> Path:
    base_dir = tmp_path / "chik_project"
    base_dir.mkdir()
    chik_dir = base_dir / "chik"
    chik_dir.mkdir()

    # Create some files within the chik package
    create_python_file(chik_dir, "module1.py", "def func1(): pass")
    create_python_file(chik_dir, "module2.py", "def func2(): pass\nfrom chik.module1 import func1")
    create_python_file(chik_dir, "module3.py", "def func3(): pass\nimport chik.module2")

    return chik_dir


def test_build_dependency_graph(chik_package_structure: Path) -> None:
    chik_dir = chik_package_structure
    dir_params = DirectoryParameters(dir_path=chik_dir)
    graph = build_dependency_graph(dir_params)
    assert chik_dir / "module1.py" in graph
    assert chik_dir / "module2.py" in graph
    assert chik_dir / "module3.py" in graph
    assert chik_dir / "module1.py" in graph[chik_dir / "module2.py"]
    assert chik_dir / "module2.py" in graph[chik_dir / "module3.py"]


def test_print_dependency_graph(chik_package_structure: Path) -> None:
    # Run the command
    runner = CliRunner()
    result = runner.invoke(cli, ["print_dependency_graph", "--directory", str(chik_package_structure)])
...
# Helper function to simulate ChikFile.parse for testing
def mock_chik_file_parse(path: Path) -> ChikFile:
    annotations_map = {
    ...
    return ChikFile(path=Path(path), annotations=annotations_map.get(path, Annotation("chik-blockchain", False)))


@pytest.fixture
def prepare_mocks(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("chik.util.virtual_project_analysis.build_dependency_graph", mock_build_dependency_graph)
    monkeypatch.setattr("chik.util.virtual_project_analysis.ChikFile.parse", mock_chik_file_parse)
...
def test_print_virtual_dependency_graph(tmp_path: Path) -> None:
    chik_dir = tmp_path / "chik"
    chik_dir.mkdir()

    # Create some files within the chik package
    create_python_file(chik_dir, "module1.py", "# Package: one\ndef func1(): pass")
    create_python_file(chik_dir, "module2.py", "# Package: two\ndef func2(): pass\nfrom chik.module1 import func1")
    create_python_file(chik_dir, "module3.py", "# Package: three\ndef func3(): pass\nimport chik.module2")

    # Run the command
    runner = CliRunner()
    result = runner.invoke(cli, ["print_virtual_dependency_graph", "--directory", str(chik_dir)])
...
# Helper function to simulate ChikFile.parse for testing
def mock_chik_file_parse2(path: Path) -> ChikFile:
    annotations_map = {
        Path("/path/to/package1/module1.py"): Annotation("Package1", True),
        Path("/path/to/package2/module2.py"): Annotation("Package2", True),
...
def prepare_mocks2(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("chik.util.virtual_project_analysis.ChikFile.parse", mock_chik_file_parse2)
...
def test_print_cycles(tmp_path: Path) -> None:
    ...
    # Create some files within the chik package
    create_python_file(chik_dir, "module1.py", "# Package: one\ndef func1(): pass\nfrom chik.module2 import func2")
    create_python_file(chik_dir, "module2.py", "# Package: two\ndef func2(): pass\nfrom chik.module3 import func3")
    create_python_file(chik_dir, "module3.py", "# Package: one\ndef func3(): pass\n")

    # Run the command
    runner = CliRunner()
    result = runner.invoke(cli, ["print_cycles", "--directory", str(chik_dir)])
...
def test_check_config(tmp_path: Path) -> None:
    ...
    create_python_file(
        ...
        textwrap.dedent(
            ...
            def func2(): pass
            from chik.module3 import func3
            from chik.module1b import func1b
            """
        ),
    )
    create_python_file(
    ...
    result = runner.invoke(
        ...
        [
            ...
            str(chik_dir / "module3.py"),
            "--ignore-specific-edge",
            str(chik_dir / "module2.py") + " -> " + str(chik_dir / "module3.py"),
            "--ignore-specific-edge",
            str(chik_dir / "module2.py") + " -> " + str(chik_dir / "module1b.py"),
        ],
    )
    assert "    module three ignored but no cycles were found" in result.output
    assert f"    file {chik_dir / 'module3.py'!s} ignored but no cycles were found" in result.output
    assert (
        f"edge {str(chik_dir / 'module2.py') + ' -> ' + str(chik_dir / 'module3.py')} ignored but no cycles were found"
        in result.output
    )
    assert (
        f"edge {str(chik_dir / 'module2.py') + ' -> ' + str(chik_dir / 'module1b.py')} ignored but no cycles were found"
...
def test_ignore_cycles_with_specific_edges(monkeypatch: pytest.MonkeyPatch) -> None:
    def _mock_chik_file_parse(path: Path) -> ChikFile:
        annotations_map = {
    ...
    monkeypatch.setattr("chik.util.virtual_project_analysis.ChikFile.parse", _mock_chik_file_parse)
...
def test_parse_edges(tmp_path: Path) -> None:
    ...
    assert result.output.strip() == f"{chik_dir / 'module1.py'!s} (one) -> {chik_dir / 'module2.py'!s} (two)"
