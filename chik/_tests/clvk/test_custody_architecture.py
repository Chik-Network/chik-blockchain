...
def test_back_and_forth_hint_parsing(restrictions: list[Restriction[MemberOrDPuz]], puzzle: MIPSComponent) -> None:
    """
    This tests that a PuzzleWithRestrictions can be exported to a clvk program to be reimported from.
...
async def test_m_of_n(cost_logger: CostLogger, with_restrictions: bool) -> None:
    ...
    async with sim_and_client(defaults=DEFAULT_CONSTANTS.replace(MAX_BLOCK_COST_CLVK=uint64(5_000_000_000))) as (
        sim,
        client,
    ):
