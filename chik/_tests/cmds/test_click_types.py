...
def test_click_address_type() -> None:
    context = cast(Context, FakeContext(obj={"expected_prefix": "xck"}))  # this makes us not have to use a config file
...
def test_click_address_type_config(root_path_populated_with_config: Path) -> None:
    ...
    assert context.obj["expected_prefix"] == "xck"  # validate that the prefix was set correctly
    # use txck address
    with pytest.raises(AttributeError):  # attribute error because the context does not have a real error handler
        AddressParamType().convert(burn_address_txck, None, context)
