...
from chik.rpc.wallet_request_types import CombineCoins, CombineCoinsResponse, SplitCoins, SplitCoinsResponse
...
def test_coins_combine(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        f"To get status, use command: chik wallet get_transaction -f {FINGERPRINT} -tx 0x{STD_TX.name.hex()}",
    ]
