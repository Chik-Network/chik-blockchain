...
from chik._tests.cmds.cmd_test_utils import TestFullNodeRpcClient, TestRpcClients, logType, run_cli_command_and_assert
from chik._tests.cmds.wallet.test_consts import get_bytes32
from chik.cmds import peer_funcs
...
def test_peer_rejects_unknown_service(service_name: str, root_path_populated_with_config: Path) -> None:
    runner = CliRunner()
    context = ChikCliContext(root_path=root_path_populated_with_config)
...
async def test_get_any_service_client_missing_config_section(
    ...
    with monkeypatch.context() as m:
        m.setattr("chik.cmds.cmds_util.load_config", lambda *_args, **_kwargs: config)
