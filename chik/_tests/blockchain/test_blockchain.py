...
from chik.consensus.block_body_validation import ForkAdd, ForkInfo
...
from chik.simulator.block_tools import BlockTools, create_block_tools_async, make_unfinished_block
...
class TestGenesisBlock:
    ...
    async def test_genesis_validate_1(
        ...
        monkeypatch.setattr(
            "chik.consensus.multiprocess_validation.pre_sp_tx_block_height",
...
class TestBlockHeaderValidation:
    ...
    # --- a/chik/consensus/multiprocess_validation.py
    # +++ b/chik/consensus/multiprocess_validation.py
    ...
    async def test_height(self, empty_blockchain: Blockchain, bt: BlockTools, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "chik.consensus.blockchain_mmr.BlockchainMMRManager.add_block_to_mmr",
...
class TestBodyValidation:
    ...
    async def test_prevalidation_fast_fail(
        ...
        # the generator for malicious CLVK. Prevalidation must reject this with
        # the cheap generator_root hash check
        # (INVALID_TRANSACTIONS_GENERATOR_HASH) BEFORE executing the generator,
        # so the expensive CLVK run is never performed for the bad block. This
        # mirrors the unfinished-block path, which already gates CLVK on these
        ...
        # INVALID_TRANSACTIONS_GENERATOR_HASH instead proves CLVK did not run.
        def _trap_run_block(*args: object, **kwargs: object) -> None:
            raise AssertionError("CLVK generator execution must not run for a mutated generator")

        monkeypatch.setattr("chik.consensus.multiprocess_validation._run_block", _trap_run_block)
        ...
        # the block before CLVK execution.
        mutated = replace_generator(block, malicious_generator)
        ...
        monkeypatch.setattr("chik.consensus.multiprocess_validation.get_block_generator", _fail_generator_resolution)
    ...
    async def test_cost_exceeds_max(
        ...
        npc_result = get_name_puzzle_conditions(
            block_generator,
            b.constants.MAX_BLOCK_COST_CLVK * 1000,
            mempool_mode=False,
    ...
    @pytest.mark.anyio
    async def test_clvk_must_not_fail(self, empty_blockchain: Blockchain, bt: BlockTools) -> None:
        # 8
        pass
    ...
    async def test_invalid_cost_in_block(
        ...
        npc_result = get_name_puzzle_conditions(
            block_generator,
            min(b.constants.MAX_BLOCK_COST_CLVK * 1000, block.transactions_info.cost),
            mempool_mode=False,
            height=softfork_height,
            constants=bt.constants,
        )
        ...
        assert generator is not None
        block_generator = BlockGenerator(generator, [])
        max_cost = min(b.constants.MAX_BLOCK_COST_CLVK * 1000, block.transactions_info.cost)
        npc_result = get_name_puzzle_conditions(
            block_generator,
            max_cost,
            mempool_mode=False,
        ...
        # when the CLVK program exceeds cost during execution, it will fail with
...
class TestReorgs:
    ...
    async def test_long_reorg(
        ...
        monkeypatch.setattr("chik.consensus.block_header_validation.validate_vdf", lambda *a, **kw: True)
        monkeypatch.setattr("chik.consensus.block_header_validation.AugSchemeMPL.verify", lambda *a, **kw: True)
        if light_blocks:
...
class ForkInfoTestSetup:
    ...
    def create(cls, same_ph_as_parent: bool, same_amount_as_parent: bool) -> ForkInfoTestSetup:
        from chik._tests.util.network_protocol_data import full_block as test_block
