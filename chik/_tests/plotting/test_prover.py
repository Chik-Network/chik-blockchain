...
from chik.plotting.prover import PlotVersion, V1Prover, V2Prover, V2Quality, get_prover_from_bytes, get_prover_from_file
...
class TestActiveV2Prover:
    def test_get_qualities_deduplicates_repeated_partial_proofs(self) -> None:
        ...
        with patch.dict("chik._tests.util.plot_cache._qualities", {}, clear=True):
...
class TestGetProverFromBytes:
    def test_get_prover_from_bytes_v1_plot(self) -> None:
        with patch("chik.plotting.prover.DiskProver") as mock_disk_prover_class:
            mock_disk_prover = MagicMock()
