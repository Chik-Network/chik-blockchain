...
def test_offer_enforces_remaining_max_cost_across_spends() -> None:
    ...
    coin_generator = CoinGenerator()
    # ~6.3e9 each; MAX_BLOCK_COST_CLVK is 11e9, so one fits and two do not.
    spends = [
        make_spend(
...
def test_offer_maps_clvk_cost_exceeded_value_error(monkeypatch: pytest.MonkeyPatch) -> None:
    """After prior spends leave almost no CLVK budget, run_with_cost raises ValueError.

    Offer remaps that to ValidationError. Lower MAX_BLOCK_COST_CLVK so a cheap
    first spend can deplete the budget without needing huge CREATE_COIN lists.
    """
    from chik_rs import G2Element
    from chik_rs.sized_ints import uint64

    import chik.wallet.trading.offer as offer_mod
    from chik.wallet.trading.offer import Offer
    from chik.wallet.wallet_spend_bundle import WalletSpendBundle

    coin_generator = CoinGenerator()
    # Minimal first spend (~44 cost). Leave only 10 for the second so CLVK aborts
    ...
    monkeypatch.setattr(
        offer_mod,
        "DEFAULT_CONSTANTS",
        DEFAULT_CONSTANTS.replace(MAX_BLOCK_COST_CLVK=uint64(first_cost + 10)),
    )
