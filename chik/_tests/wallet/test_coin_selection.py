...
class TestCoinSelection:
    ...
    async def test_coin_selection_with_primary_coin(self, wallet_environments: WalletTestFramework) -> None:
        ...
        whole_balance = uint64(await env.xck_wallet.get_confirmed_balance())
        async with env.wallet_state_manager.new_action_scope(
            wallet_environments.tx_config,
            push=False,
        ) as action_scope:
            selected_coins = await env.xck_wallet.select_coins(whole_balance, action_scope)
        ...
        async with env.wallet_state_manager.new_action_scope(
            ...
            await env.xck_wallet.generate_signed_transaction([whole_balance], [bytes32.zeros], action_scope)
