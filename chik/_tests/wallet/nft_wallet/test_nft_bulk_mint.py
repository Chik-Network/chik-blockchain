...
from chik.consensus.block_rewards import (
...
async def test_nft_mint_from_did(
    ...
    await server_0.start_client(
        PeerInfo(self_hostname, full_node_server.get_port()), None
    )
...
async def test_nft_mint_from_did_rpc(
    ...
    async with contextlib.AsyncExitStack() as exit_stack:
        ...
        hmr_did_id = encode_puzzle_hash(
            bytes32.from_hexstr(hex_did_id), AddressType.DID.hrp(config)
        )

        await full_node_api.wait_for_wallet_synced(
            wallet_node=wallet_node_maker, timeout=20
        )
        ...
        nft_wallet_taker = await api_taker.create_new_wallet(
            dict(wallet_type="nft_wallet", name="NFT WALLET 2")
        )
        ...
        for i in range(0, n, chunk):
            ...
            did_lineage_parent = next(
                cn for cn in sb.removals() if cn.name() == did_coin.name()
            ).parent_coin_info.hex()
            did_coin = next(
                cn
                for cn in sb.additions()
        ...
        async def get_taker_nfts() -> int:
            return int(
                (
                    await api_taker.nft_count_nfts(
                        {"wallet_id": nft_wallet_taker["wallet_id"]}
                    )
                )["count"]
            )
        ...
        nfts = (
            await api_taker.nft_get_nfts({"wallet_id": nft_wallet_taker["wallet_id"]})
        )["nft_list"]
        for nft in nfts:
...
async def test_nft_mint_from_did_multiple_xck(
    ...
    async with wallet_maker.wallet_state_manager.new_action_scope(
        ...
        xck_coins_1 = await wallet_maker.select_coins(
            amount=10000, action_scope=action_scope
        )
    ...
    await time_out_assert_not_none(
        5, full_node_api.full_node.mempool_manager.get_spendbundle, sb.name()
    )
    await full_node_api.farm_new_transaction_block(FarmNewBlockProtocol(ph_token))
