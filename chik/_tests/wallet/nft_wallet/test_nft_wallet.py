...
from chik.cmds.cmd_helpers import NeedsTXConfig, NeedsWalletRPC, TransactionsOut, WalletClientInfo
...
from chik.wallet.wallet_rpc_api import MAX_NFT_CHUNK_SIZE
...
async def test_nft_wallet_rpc_creation_and_list(wallet_environments: WalletTestFramework) -> None:
    ...
    royalty_address = CliAddress(
        wallet_ph,
        encode_puzzle_hash(wallet_ph, AddressType.XCK.hrp(env.node.config)),
        AddressType.XCK,
    )
...
async def test_nft_wallet_rpc_update_metadata(
    ...
    assert (
        'On-chain data/info:        ((117 "https://www.chiknetwork.com/img/branding/chik-logo.svg") '
...
async def test_nft_with_did_wallet_creation(
    ...
    await mint_nft_via_cli(
        ...
        env,
        wallet_id=nft_wallet.id(),
        target_address=CliAddress(nft_ph, encode_puzzle_hash(nft_ph, "txck"), AddressType.XCK),
        hash="0xD4584AD463139FA8C0D9F68F4B59F185D4584AD463139FA8C0D9F68F4B59F185",
        uris=["https://www.chiknetwork.com/img/branding/chik-logo.svg"],
        no_did_ownership=True,
    )
...
async def test_nft_rpc_mint(wallet_environments: WalletTestFramework) -> None:
    ...
    royalty_cli = CliAddress(
        royalty_address,
        encode_puzzle_hash(royalty_address, AddressType.XCK.hrp(env.node.config)),
        AddressType.XCK,
    )
...
async def test_nft_transfer_nft_with_did(wallet_environments: WalletTestFramework) -> None:
    ...
    await transfer_nft_via_cli(
        ...
        target_address=CliAddress(
            wallet_1_ph,
            encode_puzzle_hash(wallet_1_ph, "xck"),
            AddressType.XCK,
        ),
        fee=uint64(fee),
    )
...
async def test_nft_bulk_set_did(wallet_environments: WalletTestFramework) -> None:
    ...
    await mint_nft_via_cli(
        ...
        hash="0xD4584AD463139FA8C0D9F68F4B59F185D4584AD463139FA8C0D9F68F4B59F185",
        uris=["https://www.chiknetwork.com/img/branding/chik-logo.svg"],
        metadata_uris="https://www.chiknetwork.com/img/branding/chik-logo.svg",
    )
...
async def test_nft_bulk_transfer(wallet_environments: WalletTestFramework) -> None:
    ...
    async with wallet_1.wallet_state_manager.new_action_scope(wallet_environments.tx_config, push=True) as action_scope:
        wallet_1_ph = await action_scope.get_puzzle_hash(wallet_1.wallet_state_manager)
    fee = uint64(1000)
    address = encode_puzzle_hash(wallet_1_ph, AddressType.XCK.hrp(env_1.node.config))
    ...
    check_click_parsing(
        MintNftCMD(
            **{
                **STANDARD_TX_ENDPOINT_ARGS,
                "wallet_id": 3,
                "royalty_address": CliAddress(target_puzzle_hash, target_address, AddressType.XCK),
