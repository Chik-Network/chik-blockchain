...
from chik.wallet.remote_wallet.remote_info import RemoteInfo
...
from chik.wallet.wallet_request_types import RegisterRemoteCoins
...
async def test_wallet_state_manager_loads_remote_wallet_on_restart(
    ...
    async with wsm.lock:
        remote_wallet = await RemoteWallet.create_new_remote_wallet(wsm, env.xck_wallet, name="Remote Wallet #1")
    await remote_wallet.register_remote_coins([coin_id])
