...
from chik.cmds.cmd_helpers import NeedsTXConfig, NeedsWalletRPC, TransactionsOut
...
async def test_create_new_did_wallet_failures_no_orphan(
    ...
    """
    Regression test for https://github.com/Chik-Network/chik-blockchain/pull/20575
...
async def test_creation_from_backup_file(wallet_environments: WalletTestFramework) -> None:
    ...
    for env in (env_0, env_1, env_2):
        env.wallet_aliases = {"xck": 1, "did": 2, "nft": 3}
...
async def test_did_find_lost_did(wallet_environments: WalletTestFramework, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    env_0.wallet_aliases = {"xck": 1, "did": 2, "nft": 3}
    did_wallet_0 = await create_did_via_cli(wallet_environments, env_0, amount=101)
    await wallet_environments.process_pending_states(
        [
            WalletStateTransition(
...
async def test_did_transfer(wallet_environments: WalletTestFramework, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    env_0.wallet_aliases = {"xck": 1, "did": 2, "nft": 3}
    env_1.wallet_aliases = {"xck": 1, "did": 2, "nft": 3}
    fee = uint64(1000)
    ...
    await wallet_environments.process_pending_states(
        [
            ...
            WalletStateTransition(),
            WalletStateTransition(
                pre_block_balance_updates={"xck": {"set_remainder": True}, "did": {"set_remainder": True}},
                post_block_balance_updates={"xck": {"set_remainder": True}, "did": {"set_remainder": True}},
            ),
        ]
    )
    ...
    target_address = CliAddress(
        new_puzhash,
        encode_puzzle_hash(new_puzhash, AddressType.XCK.hrp(env_1.node.config)),
        AddressType.XCK,
    )
...
async def test_get_info(wallet_environments: WalletTestFramework, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    async with env_0.wallet_state_manager.new_action_scope(wallet_environments.tx_config, push=True) as action_scope:
        coin = (await env_0.xck_wallet.select_coins(uint64(1), action_scope)).pop()
    coin_id = coin.name()
    capsys.readouterr()
    await DidGetDetailsCMD(
        rpc_info=wallet_environments.cmd_tx_endpoint_args(env_0)["rpc_info"],
    ...
    async with env_0.wallet_state_manager.new_action_scope(wallet_environments.tx_config, push=True) as action_scope:
        ...
        coin_1 = (await env_0.xck_wallet.select_coins(odd_amount, action_scope)).pop()
    async with env_0.wallet_state_manager.new_action_scope(
        ...
        await env_0.xck_wallet.generate_signed_transaction([odd_amount], [ph_1], action_scope, fee)

    await wallet_environments.process_pending_states(
        [
            WalletStateTransition(
                pre_block_balance_updates={
                    "xck": {"unconfirmed_wallet_balance": -odd_amount - fee, "set_remainder": True},
                    "did": {"set_remainder": True},
                },
                post_block_balance_updates={
                    "xck": {"confirmed_wallet_balance": -odd_amount - fee, "set_remainder": True},
                    "did": {"set_remainder": True},
                },
            ),
            WalletStateTransition(
                pre_block_balance_updates={"xck": {"unconfirmed_wallet_balance": 0, "set_remainder": True}},
                post_block_balance_updates={
                    "xck": {"confirmed_wallet_balance": odd_amount, "set_remainder": True},
                },
            ),
        ]
    )
...
async def test_message_spend(wallet_environments: WalletTestFramework, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    conditions = conditions_dict_for_solution(
        spend.puzzle_reveal, spend.solution, env.wallet_state_manager.constants.MAX_BLOCK_COST_CLVK
    )
...
async def test_update_metadata(wallet_environments: WalletTestFramework, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    await wallet_environments.process_pending_states(
        [
            WalletStateTransition(
                pre_block_balance_updates={
                    "xck": {"unconfirmed_wallet_balance": -fee, "set_remainder": True},
                    "did": {"unconfirmed_wallet_balance": 0, "set_remainder": True},
                },
                post_block_balance_updates={
                    "xck": {"confirmed_wallet_balance": -fee, "set_remainder": True},
...
async def test_did_coin_records(wallet_environments: WalletTestFramework) -> None:
    ...
    for _ in range(2):
        ...
        target_address = CliAddress(
            target_puzhash,
            encode_puzzle_hash(target_puzhash, AddressType.XCK.hrp(env.node.config)),
            AddressType.XCK,
        )
        await DidTransferDidCMD(
            **{
...
def test_did_command_parsing() -> None:
    ...
    did_address = encode_puzzle_hash(did_puzzle_hash, "did:chik:")
    target_puzzle_hash = bytes32([2] * 32)
    target_address = encode_puzzle_hash(target_puzzle_hash, "txck")
    ...
    check_click_parsing(
        ...
        "-m",
        message.hex(),
        context=ChikCliContext(expected_prefix="txck"),
    )
    ...
    check_click_parsing(
        DidTransferDidCMD(
            ...
            transaction_writer=TransactionsOut(transaction_file_out=None),
            wallet_id=3,
            target_address=CliAddress(target_puzzle_hash, target_address, AddressType.XCK),
            reset_recovery=False,
