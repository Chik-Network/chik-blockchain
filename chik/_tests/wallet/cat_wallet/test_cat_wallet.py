...
from chik.wallet.conditions import CreateCoin, UnknownCondition
...
from chik.wallet.vc_wallet.vc_drivers import create_revocation_layer
...
from chik.wallet.wallet_request_types import GetTransactionMemo, PushTX, SendTransaction
...
async def mint_cat(
    ...
    async with environment.wallet_state_manager.new_action_scope(
        ...
        await environment.xck_wallet.generate_signed_transaction(
            amounts=[amount],
    ...
    cat_wallet = await wallet_type.get_or_create_wallet_for_cat(
        environment.wallet_state_manager, environment.xck_wallet, tail_hash, *extra_args
    )
...
async def test_cat_spend(wallet_environments: WalletTestFramework, wallet_type: type[CATWallet]) -> None:
    ...
    cat_wallet = await mint_cat(wallet_environments, env_1, "xck", "cat", uint64(100), wallet_type, "cat wallet")
...
async def test_get_wallet_for_asset_id(wallet_environments: WalletTestFramework, wallet_type: type[CATWallet]) -> None:
    ...
    cat_wallet = await mint_cat(
        wallet_environments, wallet_environments.environments[0], "xck", "cat", uint64(100), wallet_type, "cat wallet"
    )
...
async def test_cat_spend_multiple(wallet_environments: WalletTestFramework, wallet_type: type[CATWallet]) -> None:
    ...
    cat_wallet_0 = await mint_cat(wallet_environments, env_0, "xck", "cat", uint64(100), wallet_type, "cat wallet")
...
async def test_cat_max_amount_send(wallet_environments: WalletTestFramework, wallet_type: type[CATWallet]) -> None:
    ...
    cat_wallet = await mint_cat(wallet_environments, env, "xck", "cat", uint64(100000), wallet_type, "cat wallet")
...
async def test_cat_puzzle_hashes(wallet_environments: WalletTestFramework, wallet_type: type[CATWallet]) -> None:
    ...
    cat_wallet = await mint_cat(wallet_environments, env, "xck", "cat", uint64(100), wallet_type, "cat wallet")
