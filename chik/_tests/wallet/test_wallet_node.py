...
import pytest
from chik_rs import CoinState, FullBlock, G1Element, HeaderBlock, PrivateKey
from chik_rs.sized_bytes import bytes32
from chik_rs.sized_ints import uint8, uint32, uint64, uint128

from chik._tests.conftest import ConsensusMode
from chik._tests.connection_utils import add_dummy_connection_wsc
...
from chik.protocols.fee_estimate import FeeEstimateGroup
...
from chik.wallet.wallet_node import Balance, WalletNode, request_and_validate_header_block
...
async def test_get_timestamp_for_height_from_peer_backtracks_to_tx_block_deterministic(
    ...
    import chik.wallet.wallet_node as wallet_node_module
...
async def test_get_timestamp_for_height_from_peer_limits_backtrack(
    ...
    timestamp = await wallet_node.get_timestamp_for_height_from_peer(
        uint32(start_height), cast(WSChikConnection, object())
    )
    ...
    assert (
        await wallet_node.get_timestamp_for_height_from_peer(uint32(start_height), cast(WSChikConnection, object()))
        == timestamp_at_limit
    )
    assert requested_heights == list(range(start_height, start_height - max_backtrack, -1))
...
async def test_get_timestamp_for_height_from_peer_rejects_wrong_response_height(
    ...
    peer = cast(WSChikConnection, types.SimpleNamespace(get_peer_info=lambda: "peer"))
    add_to_blocks = Mock()
    ...
    async def wrong_height_response(peer: WSChikConnection, start_height: uint32, end_height: uint32) -> list[object]:
        return [
    ...
    monkeypatch.setattr("chik.wallet.wallet_node.request_header_blocks", wrong_height_response)
...
async def test_get_timestamp_for_height_from_peer_rejects_missing_peak_response(
    ...
    async def missing_response(peer: WSChikConnection, start_height: uint32, end_height: uint32) -> list[object] | None:
        requested_heights.append(int(start_height))
        return peak_response

    monkeypatch.setattr(wallet_node, "get_cache_for_peer", lambda peer: cache)
    monkeypatch.setattr("chik.wallet.wallet_node.request_header_blocks", missing_response)
...
async def test_get_timestamp_for_height_from_peer_unanchored_backtracks_past_missing_peak(
    ...
    async def response(peer: WSChikConnection, start_height: uint32, end_height: uint32) -> list[object]:
        requested_heights.append(int(start_height))
    ...
    monkeypatch.setattr(wallet_node, "get_cache_for_peer", lambda peer: cache)
    monkeypatch.setattr("chik.wallet.wallet_node.request_header_blocks", response)
...
async def test_get_timestamp_for_height_from_peer_rejects_multiple_response_blocks(
    ...
    async def multiple_block_response(peer: WSChikConnection, start_height: uint32, end_height: uint32) -> list[object]:
        return [
            types.SimpleNamespace(
                height=uint32(start_height),
                header_hash=expected_hash,
    ...
    monkeypatch.setattr(wallet_node, "get_cache_for_peer", lambda peer: cache)
    monkeypatch.setattr("chik.wallet.wallet_node.request_header_blocks", multiple_block_response)
...
async def test_get_timestamp_for_height_from_peer_validates_backtrack_chain(
    ...
    monkeypatch.setattr("chik.wallet.wallet_node.request_header_blocks", request_header_blocks_mock)
...
async def test_get_timestamp_for_height_from_peer_refetches_stale_cached_block(
    ...
    monkeypatch.setattr(wallet_node, "get_cache_for_peer", lambda peer: cache)
    monkeypatch.setattr(
        "chik.wallet.wallet_node.request_header_blocks", AsyncMock(side_effect=[[top_block], [parent_block]])
    )
...
async def test_request_fee_estimates(simulator_and_wallet: OldSimulatorsAndWallets, self_hostname: str) -> None:
    ...
    assert int(estimates.estimates[0].estimated_fee_rate.mojos_per_clvk_cost) >= 0
...
def make_backtrack_test_node(height: int, *, has_peak: bool = False) -> WalletNode:
    node = WalletNode(
        ...
        root_path=Path(".chik_wallet_node_tests_root"),
...
) -> tuple[WalletNode, WSChikConnection, AsyncMock]:
...
async def test_transaction_send_cache(self_hostname: str, simulator_and_wallet: OldSimulatorsAndWallets) -> None:
    ...
    async def send_transaction(
        self: Self, request: wallet_protocol.SendTransaction, peer: WSChikConnection, *, test: bool = False
    ) -> Message | None:
...
async def test_transaction_ack_duplicate_without_resend_ignored(
    ...
    caplog.clear()
    with caplog.at_level(logging.DEBUG, logger="chik.wallet.wallet_node"):
        for _ in range(10):
            await conn.incoming_queue.put(msg)
...
async def test_process_new_subscriptions_continues_after_peer_error(
    ...
    async def register_for_ph_updates(
        self: object, request: wallet_protocol.RegisterForPhUpdates, peer: WSChikConnection
    ) -> None:
        raise RuntimeError("simulated peer failure")

    async def register_for_coin_updates(
        self: object, request: wallet_protocol.RegisterForCoinUpdates, peer: WSChikConnection
    ) -> None:
        raise RuntimeError("simulated peer failure")
