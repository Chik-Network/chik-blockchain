...
import pytest
from chik_rs import AugSchemeMPL, Coin, CoinSpend, G1Element, G2Element
...
async def run_send_cmd(
    ...
    *,
    ...
    wallet_id = int(env.xck_wallet.id())
...
class TestWalletSimulator:
    ...
    async def test_wallet_make_transaction(self, wallet_environments: WalletTestFramework) -> None:
            ...
            **{
                **wallet_environments.cmd_tx_endpoint_args(env),
                ...
                "address": CliAddress(
                    ...
                    original_address=encode_puzzle_hash(bytes32.zeros, "txck"),
                    address_type=AddressType.XCK,
                ),
                "override": False,
                "clawback_time": 0,
        ...
        async with env.wallet_state_manager.new_action_scope(wallet_environments.tx_config, push=False) as action_scope:
            selected_coin = next(iter(await env.xck_wallet.select_coins(uint64(0), action_scope)))
        assert await env.xck_wallet.match_hinted_coin(selected_coin, selected_coin.puzzle_hash)
    ...
    async def test_wallet_clawback_claim_auto(
        ...
        tx_amount = 500
        async with env_1.xck_wallet.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
            normal_puzhash = await action_scope.get_puzzle_hash(env_1.xck_wallet.wallet_state_manager)
...
def test_wallet_command_parsing() -> None:
    ...
    address = encode_puzzle_hash(puzzle_hash, "txck")
    cli_address = CliAddress(puzzle_hash=puzzle_hash, original_address=address, address_type=AddressType.XCK)
    ...
    check_click_parsing(
        CheckWalletCMD(context=ChikCliContext(), verbose=True, db_path="wallet.sqlite"),
        "-v",
        "--db-path",
        "wallet.sqlite",
    )
