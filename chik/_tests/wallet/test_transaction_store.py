...
tr1 = TransactionRecord(
    ...
    to_address=encode_puzzle_hash(bytes32(bytes32.zeros), "txck"),
    amount=uint64(1234),
...
MINIMUM_CONFIG = {
    "network_overrides": {"config": {"testnet": {"address_prefix": "txck"}}},
...
async def test_get_transactions_between_to_puzzle_hash(seeded_random: random.Random) -> None:
    async with DBConnection(1) as db_wrapper:
        ...
        ad1 = encode_puzzle_hash(ph1, "txck")
        ph2 = bytes32.random(seeded_random)
        ad2 = encode_puzzle_hash(ph2, "txck")
