...
import aiosqlite
import pytest
from chik_rs import CoinRecord, CoinSpend, G1Element, G2Element
...
from chik.pools.pool_wallet_info import NewPoolWalletInitialTargetState
from chik.protocols.fee_estimate import FeeEstimate, FeeEstimateGroup
...
from chik.wallet.util.clvk_streamable import byte_deserialize_clvk_streamable
...
from chik.wallet.vc_wallet.cr_cat_drivers import CRCATMetadata
...
from chik.wallet.wallet_rpc_metadata import WalletRpcMetadata
from chik.wallet.wallet_spend_bundle import WalletSpendBundle
from chik.wallet.wallet_state_manager import SyncStatus
...
async def test_send_transaction(wallet_environments: WalletTestFramework) -> None:
    ...
    wallet_2: Wallet = env_2.xck_wallet
    ...
    INITIAL_FUNDS = await env.xck_wallet.get_confirmed_balance()
    async with wallet_2.wallet_state_manager.new_action_scope(wallet_environments.tx_config, push=True) as action_scope:
        addr = encode_puzzle_hash(await action_scope.get_puzzle_hash(wallet_2.wallet_state_manager), "txck")
...
async def test_create_signed_transaction(
    ...
    env.wallet_aliases = {"xck": 1, "cat": 2}
    env_2.wallet_aliases = {"xck": 1, "cat": 2}
    ...
    xck_delta = amount_total if not is_cat else amount_fee
        ...
        [
            WalletStateTransition(
                pre_block_balance_updates={  # type: ignore[arg-type]
                    "xck": {
                        "unconfirmed_wallet_balance": -xck_delta,
                        "<=#spendable_balance": -xck_delta,
                        "<=#max_send_amount": -xck_delta,
                        ">=#pending_change": 0,
                ...
                post_block_balance_updates={  # type: ignore[arg-type]
                    "xck": {
                        "confirmed_wallet_balance": -xck_delta,
                        ">=#spendable_balance": 0,
                        ">=#max_send_amount": 0,
                        "<=#pending_change": 0,
                        "pending_coin_removal_count": -1,
...
async def test_create_signed_transaction_with_excluded_coins(wallet_environments: WalletTestFramework) -> None:
    env = wallet_environments.environments[0]
    wallet_1: Wallet = env.xck_wallet
    wallet_1_rpc: WalletRpcClient = env.rpc_client
    full_node_rpc: FullNodeRpcClient = wallet_environments.full_node_rpc_client
...
async def test_send_transaction_multi(wallet_environments: WalletTestFramework) -> None:
    ...
    INITIAL_BALANCE = await env.xck_wallet.get_confirmed_balance()
...
async def test_get_transactions(wallet_environments: WalletTestFramework, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
    await client.send_transaction(
        SendTransaction(wallet_id=uint32(1), amount=uint64(1), address=encode_puzzle_hash(puzhash, "txck"), push=True),
        wallet_environments.tx_config,
    )  # Create a pending tx
    ...
    await client.send_transaction(
        SendTransaction(
            wallet_id=uint32(1), amount=uint64(1), address=encode_puzzle_hash(ph_by_addr, "txck"), push=True
        ),
        wallet_environments.tx_config,
    )
    unconfirmed_txs_count += 1
    ...
    tx_for_address = (
        await client.get_transactions(
            GetTransactions(wallet_id=uint32(1), to_address=encode_puzzle_hash(ph_by_addr, "txck"))
        )
    ).transactions
    assert (
...
async def test_offer_endpoints(
    ...
    monkeypatch.setattr("chik.cmds.wallet_funcs.cli_confirm", lambda *args, **kwargs: None)
...
async def test_get_coin_records_by_names(wallet_environments: WalletTestFramework) -> None:
    ...
    async with env.wallet_state_manager.new_action_scope(wallet_environments.tx_config, push=True) as action_scope:
        address = encode_puzzle_hash(await action_scope.get_puzzle_hash(env.wallet_state_manager), "txck")
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
...
async def test_key_and_address_endpoints(
    ...
    await CheckWalletCMD(
        context=ChikCliContext(root_path=wallet_node.root_path),
        verbose=False,
        db_path=str(db_path),
    ).run()
...
async def test_select_coins_rpc(wallet_environments: WalletTestFramework) -> None:
    ...
    env.wallet_aliases = {"xck": 1}
    env_2.wallet_aliases = {"xck": 1}

    wallet_2: Wallet = env_2.xck_wallet
    client: WalletRpcClient = env.rpc_client
    client_2: WalletRpcClient = env_2.rpc_client

    funds: int = await env.xck_wallet.get_confirmed_balance()
...
async def test_sign_message_by_address(
    ...
    await SignMessageCMD(
        rpc_info=rpc_info,
        address=CliAddress(decode_puzzle_hash(address), address, AddressType.XCK),
        hex_message=message,
    ).run()
...
async def test_set_wallet_resync_on_startup(wallet_environments: WalletTestFramework) -> None:
    ...
    address = (await wc.get_next_address(GetNextAddress(wallet_id=env.xck_wallet.id(), new_address=True))).address
    await wc.mint_nft(
        request=NFTMintNFTRequest(
            wallet_id=nft_wallet_id,
...
async def test_combine_coins_respects_max_coin_amount(wallet_environments: WalletTestFramework) -> None:
    ...
    Setup gives 4 coins: 2x 1.75 XCK + 2x 0.25 XCK.
    With largest_first=True and NO filter, the two 1.75 XCK coins would be
    selected.  With max_coin_amount=0.25 XCK the 1.75 XCK coins must be
    excluded and only the 0.25 XCK coins selected -- proving the filter works.
    """
    env = wallet_environments.environments[0]
    env.wallet_aliases = {"xck": 1}

    SMALL = uint64(250_000_000_000)  # 0.25 XCK
...
async def test_combine_coins_rejects_spent_coins(wallet_environments: WalletTestFramework) -> None:
    ...
    # Grab a 0.25 XCK coin and remember its ID before spending it
    async with env.wallet_state_manager.new_action_scope(wallet_environments.tx_config) as action_scope:
        target_coin = next(iter(await env.xck_wallet.select_coins(SMALL, action_scope)))
    ...
    # Wallet now has 3 unspent coins (2x 1.75 XCK + 1x 0.5 XCK) and 2 spent
    # 0.25 XCK records still in the DB.  A fill-up combine (no explicit IDs,
    # smallest-first) must skip the spent records and select only unspent coins.
    # Without the spent_range fix this would silently pick the spent 0.25 XCK
    # records because they sort before the 0.5 XCK coin.
    COMBINED = uint64(2 * SMALL)  # 0.5 XCK from first combine
    BIG = uint64(1_750_000_000_000)  # 1.75 XCK
...
def test_send_transaction_multi_post_init() -> None:
    with pytest.raises(
        ValueError,
        match=re.escape('Specified "morph_bytes" for a CAT-type wallet. Maybe you meant to specify an XCK wallet?'),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
            morph_bytes=b"",
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            'Specified "coin/puzzle_announcements" for a CAT-type wallet.Maybe you meant to specify an XCK wallet?'
        ),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
    ...
    with pytest.raises(
        ValueError,
        match=re.escape('Specified "amount" for an XCK wallet. Maybe you meant to specify a CAT-type wallet?'),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
            amount=uint64(100),
    ...
    with pytest.raises(
        ValueError,
        match=re.escape('Specified "inner_address" for an XCK wallet. Maybe you meant to specify a CAT-type wallet?'),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
            inner_address="foo",
    ...
    with pytest.raises(
        ValueError,
        match=re.escape('Specified "memos" for an XCK wallet. Maybe you meant to specify a CAT-type wallet?'),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
            memos=["foo"],
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            'Specified "extra_delta", "tail_reveal", or "tail_solution" for an XCK wallet.'
    ...
    with pytest.raises(
        ValueError,
        match=re.escape('"additions" are required for XCK wallets.'),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
        ).convert_to_proxy(CreateSignedTransaction)
