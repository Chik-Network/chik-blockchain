    ...
    for connection_type in NodeType:
        if connection_type != NodeType.FARMER:
            with pytest.raises(InvalidConnectionTypeError):
                dummy_connection: WSChikConnection = get_dummy_connection(
...
def test_process_batch_uses_plot_serialization_for_farmer_protocol(
    ...
    farmer_connection = cast(WSChikConnection, get_dummy_connection(NodeType.FARMER, bytes32.random(seeded_random)))
