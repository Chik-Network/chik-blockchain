...
async def send_sb(node: FullNodeAPI, dummy_peer: WSChikConnection, sb: SpendBundle) -> Message | None:
    tx = wallet_protocol.SendTransaction(sb)
    return await node.send_transaction(tx, dummy_peer, test=True)


async def gen_and_send_sb(
    node: FullNodeAPI, dummy_peer: WSChikConnection, wallet: WalletTool, coin: Coin, fee: uint64 = uint64(0)
) -> SpendBundle:
...
class TestMempoolManager:
    ...
    async def condition_tester(
        ...
        num_blocks: int = 3,
        coin: Coin | None = None,
    ) -> tuple[list[FullBlock], SpendBundle, WSChikConnection, MempoolInclusionStatus, Err | None]:
        reward_ph = wallet_a.get_new_puzzlehash()
