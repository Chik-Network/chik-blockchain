...
import pytest
from chik_rs.datalayer import KeyAlreadyPresentError, MerkleBlob, TreeIndex
...
from chik.data_layer.data_layer_errors import KeyNotFoundError, MerkleBlobNotFoundError, TreeGenerationIncrementingError
...
from chik.data_layer.util.benchmark import generate_datastore
...
async def test_insert_batch_reference_and_side(
    ...
    assert len(nodes) == 3
    assert isinstance(nodes[1], chik_rs.datalayer.LeafNode)
    assert isinstance(nodes[2], chik_rs.datalayer.LeafNode)
...
async def test_proof_of_inclusion_by_hash(data_store: DataStore, store_id: bytes32) -> None:
    ...
    expected_layers = [
        chik_rs.datalayer.ProofOfInclusionLayer(
    ...
    assert proof == chik_rs.datalayer.ProofOfInclusion(node_hash=node.hash, layers=expected_layers)
...
async def test_proof_of_inclusion_by_hash_no_ancestors(data_store: DataStore, store_id: bytes32) -> None:
    ...
    assert proof == chik_rs.datalayer.ProofOfInclusion(node_hash=node.hash, layers=[])
...
async def write_tree_to_file_old_format(
    ...
    if isinstance(raw_node, chik_rs.datalayer.InternalNode):
    ...
    elif isinstance(raw_node, chik_rs.datalayer.LeafNode):
...
async def test_insert_into_data_store_from_file_line_graph_depth(
    ...
    with pytest.raises(chik_rs.datalayer.RecursionDepthExceededError):
        await data_store.insert_into_data_store_from_file(store_id, root_hash, filename)
...
async def test_insert_into_data_store_from_file_fails_for_dag(
    ...
    root_hash = create_graph_util(filename, edges)
    with pytest.raises(chik_rs.datalayer.CycleFoundError):
        await data_store.insert_into_data_store_from_file(store_id, root_hash, filename)
        ...
        for index, store_id in enumerate(store_ids):
            ...
            for key, value in zip(keys, values):
                ...
                if random.randint(0, 4) == 0:
                    ...
                    await raw_data_store.insert_batch(store_id, batch, status=Status.COMMITTED)
                    with pytest.raises(chik_rs.datalayer.UnknownKeyError):
...
async def test_insert_from_delta_file_exceeds_max_limit(
    ...
    success = await insert_from_delta_file(
        ...
        server_info=ServerInfo("http://127.0.0.1/9237", 0, 0),
        client_foldername=tmp_path,
        timeout=aiohttp.ClientTimeout(total=15, sock_connect=5),
        log=log,
        proxy_url="",
