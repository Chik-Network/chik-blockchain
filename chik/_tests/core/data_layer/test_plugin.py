...
from chik.data_layer.data_layer import get_plugin_info
from chik.data_layer.data_layer_util import PluginRemote, ServerInfo
from chik.data_layer.download_data import download_file
...
async def _mock_session_post(response: MagicMock) -> AsyncIterator[MagicMock]:
    ...
    with patch("chik.data_layer.data_layer.aiohttp.ClientSession", return_value=session):
        yield session
...
async def _mock_session_post_download(response: MagicMock) -> AsyncIterator[MagicMock]:
    ...
    with patch("chik.data_layer.download_data.aiohttp.ClientSession", return_value=session):
        yield session
...
async def test_get_plugin_info_timeout() -> None:
    ...
    with patch(
        "chik.data_layer.data_layer.aiohttp.ClientSession",
...
async def test_download_file_plugin_timeout(tmp_path: Path) -> None:
    ...
    with patch(
        "chik.data_layer.download_data.aiohttp.ClientSession",
