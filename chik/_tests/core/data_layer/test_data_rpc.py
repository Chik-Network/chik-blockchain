...
from chik.data_layer.data_layer_service import DataLayerService
...
from chik.data_layer.start_data_layer import create_data_layer_service
...
async def process_for_data_layer_keys(
    ...
    for sleep_time in backoff_times():
        ...
        except chik_rs.datalayer.UnknownKeyError:
            pass
        else:
...
async def test_fetch_and_validate_skips_heal_when_local_ahead(
    ...
    async with init_data_layer(
        ...
        await store.remove_subscriptions(store_id, ["http://127.0.0.1/9234"])
...
def populate_reference(count: int, keys_to_prove: int) -> ProofReference:
    ret = ProofReference(
        ...
        verify_proof_response={
            "current_root": True,
            "success": True,
            "verified_clvk_hashes": {
                "store_id": "",
                "inclusions": [
                    {
                        "key_clvk_hash": "0x" + std_hash(b"\1" + value.to_bytes(length=1, byteorder="big")).hex(),
                        "value_clvk_hash": "0x"
...
async def test_dl_proof(offer_setup: OfferSetup, reference: ProofReference) -> None:
    ...
    reference.verify_proof_response["verified_clvk_hashes"]["store_id"] = f"0x{offer_setup.maker.id.hex()}"
    ...
    fakeproof = HashOnlyProof(
        key_clvk_hash=bytes32([1] * 32), value_clvk_hash=bytes32([1] * 32), node_hash=bytes32([3] * 32), layers=[]
    )
    assert fakeproof.root() == fakeproof.node_hash

    fakeproof = HashOnlyProof(
        key_clvk_hash=bytes32([1] * 32),
        value_clvk_hash=bytes32([1] * 32),
...
async def test_dl_proof_verify_errors(offer_setup: OfferSetup, seeded_random: random.Random) -> None:
    ...
    two_key_proof.verify_proof_response["verified_clvk_hashes"]["store_id"] = f"0x{offer_setup.maker.id.hex()}"
    ...
    # test bad key
    badproof = deepcopy(proof["proof"])
    badproof["store_proofs"]["proofs"][0]["key_clvk_hash"] = bytes32.random(seeded_random).hex()
    with pytest.raises(ValueError, match="Invalid Proof: node hash does not match key and value"):
        await offer_setup.taker.api.verify_proof(request=badproof)

    # test bad value
    badproof = deepcopy(proof["proof"])
    badproof["store_proofs"]["proofs"][0]["value_clvk_hash"] = bytes32.random(seeded_random).hex()
    with pytest.raises(ValueError, match="Invalid Proof: node hash does not match key and value"):
        await offer_setup.taker.api.verify_proof(request=badproof)
...
async def test_auto_subscribe_to_local_stores(
    ...
    with monkeypatch.context() as m:
        ...
        m.setattr("chik.wallet.wallet_rpc_client.WalletRpcClient.dl_track_new", mock_dl_track_new)
...
async def test_management_skips_store_without_committed_data(
    ...
    with monkeypatch.context() as m, caplog.at_level(logging.INFO):
        m.setattr("chik.wallet.wallet_rpc_client.WalletRpcClient.dl_latest_singleton", mock_dl_latest_singleton)
...
async def test_invalid_subscription_handling(
    ...
    with monkeypatch.context() as m, caplog.at_level(logging.INFO):
        m.setattr(
            "chik.data_layer.start_data_layer.WalletRpcClient.create",
            AsyncMock(
        ...
        m.setattr("chik.data_layer.data_layer.DataLayer.update_subscription", spy_update_subscription)
...
async def test_invalid_subscription_does_not_block_unsubscribe(
    ...
    with monkeypatch.context() as m:
        ...
        m.setattr("chik.data_layer.data_layer.DataLayer.update_subscription", AsyncMock(return_value=None))
        async with init_data_layer(
            wallet_rpc_port=uint16(1),
            bt=bt,
            db_path=tmp_path,
