...
from chik._tests.util.network_protocol_data import full_block
from chik.full_node.full_node import respond_blocks_or_ban
from chik.protocols.full_node_protocol import RespondBlocks
...
async def test_respond_blocks_or_ban_accepts_exact_range() -> None:
    ...
    assert await respond_blocks_or_ban(cast(WSChikConnection, peer), response, 5, 7, logging.getLogger(__name__))
...
async def test_respond_blocks_or_ban_rejects_mismatched_payloads(
    ...
    assert not await respond_blocks_or_ban(
        cast(WSChikConnection, peer), response, start_height, end_height, logging.getLogger(__name__)
    )
    assert peer.closed
