...
from chik.protocols.full_node_protocol import RespondBlock
...
async def test_sync_target_peak_gather_skips_deserialize_failure(
    ...
    bad_peer.get_peer_logging = MagicMock(return_value=PeerInfo("10.0.0.1", 9678))
    ...
    good_peer.get_peer_logging = MagicMock(return_value=PeerInfo("10.0.0.2", 9678))
    ...
    monkeypatch.setattr(
        "chik.full_node.full_node.check_fork_next_block",
...
async def test_sync_target_peak_gather_does_not_ban_on_timeout(
    ...
    slow_peer.get_peer_logging = MagicMock(return_value=PeerInfo("10.0.0.3", 9678))
