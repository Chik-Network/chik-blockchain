...
from chik.protocols.shared_protocol import Capability, ConfigureWindowSizes, default_capabilities
from chik.server.rate_limits_v3 import (
...
async def test_invalid_config_settings_length(
    ...
    monkeypatch.setattr(
        "chik.server.ws_connection.rl_v3_to_configure_message", lambda: ConfigureWindowSizes(settings=settings)
    )
...
async def test_malformed_config_message_bytes(
    ...
    original_send_message = WSChikConnection._send_message

    async def test_send_message(self: WSChikConnection, message: Message, priority: int = 0) -> None:
    ...
    monkeypatch.setattr(WSChikConnection, "_send_message", test_send_message)
