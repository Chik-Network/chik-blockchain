...
from chik_rs.sized_ints import int16, uint8, uint16, uint32
from packaging.version import Version

from chik import __version__
from chik._tests.conftest import ConsensusMode
from chik._tests.connection_utils import add_dummy_connection_wsc, connect_and_get_peer
...
from chik.protocols.shared_protocol import Capability, Error, protocol_version
...
from chik.server.ws_connection import WSChikConnection, error_response_version, sanitize_version_string
...
async def test_start_client_handshake_timeout(
    ...
    async def timeout_handshake(
        self: WSChikConnection, network_id: str, server_port: uint16, local_type: NodeType
    ) -> None:
        raise asyncio.TimeoutError

    monkeypatch.setattr(WSChikConnection, "perform_handshake", timeout_handshake)
...
async def test_start_client_handshake_timeout_configurable(
    ...
    monkeypatch.setattr("chik.server.server.is_localhost", lambda _host: False)
...
async def test_garbage_collect_connections_tolerates_stale_closed_connections(
    ...
    chik_ca_crt, chik_ca_key = chik_ssl_ca_paths(bt.root_path, bt.config)
    my_test_server = ChikServer.create(
        port=None,
        node=None,
        ...
        chik_ca_crt_key=(chik_ca_crt, chik_ca_key),
        stub_metadata_for_type={},
        name="Hello",
    )
    missing_connection = FakeConnection(PeerInfo("10.1.1.1", 9678), bytes32(b"\x01" * 32))
    stale_connection = FakeConnection(PeerInfo("10.1.1.2", 9678), bytes32(b"\x02" * 32))
    replacement_connection = FakeConnection(stale_connection.peer_info, stale_connection.peer_node_id, closed=False)
    remaining_connection = FakeConnection(PeerInfo("10.1.1.3", 9678), bytes32(b"\x03" * 32))
    missing_wsc = cast(WSChikConnection, missing_connection)
    stale_wsc = cast(WSChikConnection, stale_connection)
    replacement_wsc = cast(WSChikConnection, replacement_connection)
    remaining_wsc = cast(WSChikConnection, remaining_connection)
...
async def test_connection_closed_banning(bt: BlockTools, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING):
        ...
        exempt_peer = FakeConnection(peer_info=PeerInfo("10.1.1.1", 9678))
        await my_test_server.connection_closed(cast(WSChikConnection, exempt_peer), ban_time=60)
        ...
        localhost_peer = FakeConnection(peer_info=PeerInfo("127.0.0.1", 9678))
        await my_test_server.connection_closed(cast(WSChikConnection, localhost_peer), ban_time=60)
        assert "Trying to ban localhost for 60, but will not ban" in caplog.text

        # testing trusted peers exemption
        trusted_peer = FakeConnection(peer_info=PeerInfo("34.34.34.34", 9678))
        await my_test_server.connection_closed(cast(WSChikConnection, trusted_peer), ban_time=60)
...
async def test_route_incoming_message(
    one_node_one_block: tuple[FullNodeSimulator, ChikServer, BlockTools], self_hostname: str
) -> None:
    """
...
async def test_inbound_handshake_timeout(
    ...
    chik_ca_crt_path, chik_ca_key_path = chik_ssl_ca_paths(server_1.root_path, config)
    ...
    try:
        ...
        with patch("chik.server.server.is_localhost", return_value=False):
