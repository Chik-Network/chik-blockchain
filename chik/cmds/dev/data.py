...
from chik.wallet.wallet_request_types import DLHistory, DLTrackNew
...
class SyncTimeCommand:
    ...
    async def run(self) -> None:
        ...
        if self.restart_all:
            await self.run_chik("stop", "-d", "all", check=False)

        await self.run_chik("start", "wallet")
        await self.run_chik("keys", "generate", "--label", "for_testing")
        await self.wait_for_wallet_synced()
        await self.run_chik("wallet", "show")
    ...
    async def wait_for_wallet_synced(self) -> None:
        ...
        while True:
            run_result = await self.run_chik(
                "wallet",
                "show",
    ...
    async def run_chik(self, *args: str, check: bool = True, **kwargs: Any) -> RunResult:
        env = os.environ.copy()
        venv_path = Path(sysconfig.get_path("scripts"))
        env["PATH"] = os.pathsep.join([os.fspath(venv_path), env["PATH"]])
        env["CHIK_DATA_LAYER_STOP_AFTER_GENERATION"] = str(self.generation_limit)
        env["CHIK_ROOT"] = os.fspath(self.context.root_path)
        env["CHIK_KEYS_ROOT"] = os.fspath(self.context.keys_root_path)
