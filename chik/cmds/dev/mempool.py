...
from chik.cmds.dev.mempool_funcs import create_block_async, export_mempool_async, import_mempool_async
...
def import_mempool_cmd(ctx: click.Context, rpc_port: int | None, path: str) -> None:
    ...
    asyncio.run(import_mempool_async(rpc_port, ChikCliContext.set_default(ctx).root_path, content))
...
def export_mempool_cmd(ctx: click.Context, rpc_port: int | None, path: str) -> None:
    ...
    asyncio.run(export_mempool_async(rpc_port, ChikCliContext.set_default(ctx).root_path, path))
...
def create_block_cmd(ctx: click.Context, rpc_port: int | None) -> None:
    ...
    asyncio.run(create_block_async(rpc_port, ChikCliContext.set_default(ctx).root_path))
