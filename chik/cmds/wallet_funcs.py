...
from chik.cmds.cmd_helpers import NeedsTXConfig, WalletClientInfo
        ...
        for item in [*offers, *requests]:
                                ...
                                royalty_assets.append(
                                    RoyaltyAsset(
                                        ...
                                        royalty_address=encode_puzzle_hash(
                                            info.royalty_puzzle_hash, AddressType.XCK.hrp(config)
                                        ),
            ...
            with filepath.open(mode="w") as file:
                ...
                if res.offer is not None:
                    ...
                    print(
                        f"Use chik wallet get_offers --id {res.trade_record.trade_id} -f {fingerprint} to view status"
                    )
                else:
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: dict[str, str], has_fee: bool = False, network_xck: str = "XCK"
) -> None:
...
async def take_offer(
        ...
        if royalty_asset_id.hex() in offered:
            ...
            royalty_assets.append(
                RoyaltyAsset(
                    ...
                    royalty_address=encode_puzzle_hash(address, AddressType.XCK.hrp(config)),
    ...
    if not examine_only:
        ...
        if push:
            ...
            print(f"Use chik wallet get_offers --id {res.trade_record.trade_id} -f {fingerprint} to view its status")
...
async def send_notification(
    ...
    if push:
        ...
        print(
            "To get status, use command: chik wallet get_transaction"
