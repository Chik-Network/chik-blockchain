...
from chik.rpc.wallet_request_types import CATSpendResponse, GetNotifications, SendTransactionResponse
...
async def mint_vc(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        res = await wallet_client.vc_mint(
            ...
            CMDTXConfigLoader().to_tx_config(units["chik"], config, fingerprint),
            target_address.validate_address_type_get_ph(AddressType.XCK) if target_address else None,
            fee,
            push=push,
            timelock_info=condition_valid_times,
        )
