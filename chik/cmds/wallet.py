...
from chik.cmds.cmd_classes import ChikCliContext, argument, chik_command, get_chik_command_metadata, option
...
from chik.cmds.wallet_funcs import delete_notifications, get_notifications, send_notification
...
@chik_command(group=wallet_cmd, name="get_transaction", short_help="Get a transaction", help="Get a transaction")
...
@chik_command(group=wallet_cmd, name="get_transactions", short_help="Get all transactions", help="Get all transactions")
...
@chik_command(
    group=wallet_cmd,
    name="send",
    short_help="Send chik or other assets to another wallet",
    help="Send chik or other assets to another wallet",
)
class SendCMD(TransactionEndpoint):
    ...
    address: CliAddress = option(
        "-t", "--address", help="Address to send the XCK", type=AddressParamType(), required=True
    )
...
@chik_command(group=wallet_cmd, name="show", short_help="Show wallet information", help="Show wallet information")
class ShowCMD:
    rpc_info: NeedsWalletRPC
...
class ClawbackCMD(TransactionEndpoint):
    fee: uint64 = option(
        "-m",
        "--fee",
        help="A fee to add to the offer when it gets taken, in XCK",
        ...
        async with self.rpc_info.wallet_rpc() as wallet_info:
            return await spend_clawback(
                ...
                tx_config=self.tx_config_loader.load_tx_config(
                    units["chik"], wallet_info.config, wallet_info.fingerprint
                ),
            )
...
class SignMessageCMD:
    ...
    async def run(self) -> None:
        ...
        async with self.rpc_info.wallet_rpc() as wallet_info:
            await sign_message(
                wallet_info=wallet_info, addr_type=AddressType.XCK, message=self.hex_message, address=self.address
            )
...
@chik_command(
    group=wallet_cmd,
    name="make_offer",
    short_help="Create an offer of XCK/CATs/NFTs for XCK/CATs/NFTs",
    help="Create an offer of XCK/CATs/NFTs for XCK/CATs/NFTs",
)
class MakeOfferCMD:
    rpc_info: NeedsWalletRPC
...
class TakeOfferCMD(TransactionEndpoint):
    fee: uint64 = option(
        "-m",
        "--fee",
        help="The fee to use when pushing the completed offer, in XCK",
        type=TransactionFeeParamType(),
        default="0",
        show_default=True,
        required=True,
    ...
    async def run(self) -> list[TransactionRecord]:
        ...
        async with self.rpc_info.wallet_rpc() as wallet_info:
            return await take_offer(
                ...
                self.tx_config_loader.load_tx_config(units["chik"], wallet_info.config, wallet_info.fingerprint),
            )
...
class CancelOfferCMD(TransactionEndpoint):
    fee: uint64 = option(
        "-m",
        "--fee",
        help="The fee to use when cancelling the offer securely, in XCK",
        type=TransactionFeeParamType(),
        default="0",
        show_default=True,
        required=True,
...
@chik_command(group=wallet_cmd, name="check", short_help="Check wallet DB integrity", help=check_help_text)
