...
import aiohttp
from chik_rs import AugSchemeMPL, G2Element, PlotParam, PoolTarget, PrivateKey, ProofOfSpace
...
from chik.protocols.solver_protocol import SolverInfo, SolverResponse
...
class FarmerAPI:
    ...
    @metadata.request(peer_required=True)
    async def partial_proofs(self, partial_proof_data: PartialProofsData, peer: WSChikConnection) -> None:
        """
    ...
    @metadata.request()
    async def solution_response(self, response: SolverResponse, peer: WSChikConnection) -> None:
        """
    ...
    async def new_signage_point(self, new_signage_point: farmer_protocol.NewSignagePoint) -> None:
        ...
        try:
            ...
            def old_harvesters(conn: WSChikConnection) -> bool:
                return conn.protocol_version <= Version("0.0.36")

            def new_harvesters(conn: WSChikConnection) -> bool:
    ...
    @metadata.request(peer_required=True)
    async def respond_plots2(self, _: harvester_protocol.RespondPlots2, peer: WSChikConnection) -> None:
        self.farmer.log.warning(f"Respond plots came too late from: {peer.get_peer_logging()}")
