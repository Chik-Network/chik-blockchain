...
from chik_rs.sized_ints import int64

from chik.data_layer.data_layer_errors import (
    KeyNotFoundError,
...
@dataclass
class DataStore:
    """A key/value store with the pairs being terminal nodes in a CLVK object tree."""
    ...
    async def get_ancestors(
        ...
        for index, node in itertools.islice(lineage, 1, None):
            assert isinstance(node, chik_rs.datalayer.InternalNode)
            result.append(
                InternalNode(
                    hash=node.hash,
    ...
    async def get_terminal_node_from_kid(self, merkle_blob: MerkleBlob, kid: KeyId, store_id: bytes32) -> TerminalNode:
        ...
        assert isinstance(raw_node, chik_rs.datalayer.LeafNode)
    ...
    async def get_tree_as_nodes(self, store_id: bytes32) -> Node:
        async with self.db_wrapper.reader():
            ...
            for _, node in reversed(nodes):
                if isinstance(node, chik_rs.datalayer.InternalNode):
                    left_hash = merkle_blob.get_hash_at_index(node.left)
                    right_hash = merkle_blob.get_hash_at_index(node.right)
                    ...
                    assert isinstance(node, chik_rs.datalayer.LeafNode)
