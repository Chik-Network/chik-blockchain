...
def initialize_logging(
    ...
    if logging_config.get("log_systemd", False):
        # Prefix service name with "chik." for better filtering in journal
        # e.g. "chik.full_node", "chik.farmer"
        identifier = f"chik.{service_name}" if not service_name.startswith("chik.") else service_name
        try:
