...
class FeeEstimate(Streamable):
    ...
    estimated_fee_rate: expressed in mojo per 1 clvk_cost. `estimated_fee` can be zero.
    """
...
class FeeEstimateV2:
    error: str | None
    time_target: uint64  # unix time stamp in seconds
    estimated_fee_rate: FeeRateV2  # Mojos per clvk cost


def fee_rate_v2_to_v1(fee_rate: FeeRateV2) -> FeeRate:
    return FeeRate(uint64(math.ceil(fee_rate.mojos_per_clvk_cost)))


def fee_estimate_v2_to_v1(fe: FeeEstimateV2) -> FeeEstimate:
    return FeeEstimate(fe.error, fe.time_target, FeeRate(uint64(math.ceil(fe.estimated_fee_rate.mojos_per_clvk_cost))))
...
class FeeEstimateGroup(Streamable):
    ...
    Estimates are expressed in mojos / 1 clvk_cost.
    """
