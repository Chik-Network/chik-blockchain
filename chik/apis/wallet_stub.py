...
class WalletNodeApiStub(ApiProtocol, Protocol):
    ...
    async def transaction_ack(self, ack: TransactionAck, peer: WSChikConnection) -> None:
    ...
    @metadata.request(peer_required=True, execute_task=True)
    async def coin_state_update(self, request: CoinStateUpdate, peer: WSChikConnection) -> None:
