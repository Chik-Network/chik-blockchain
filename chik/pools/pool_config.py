...
from chik_rs.sized_byte_class import hexstr_to_bytes
...
def perform_migration_from_old_config(root_path: Path) -> None:
    with lock_and_load_config(root_path, "config.yaml") as chik_config:
        existing_pool_entries = chik_config["pool"].get("pool_list", [])
        if not PoolingShareState.state_path(root_path=root_path).exists() or existing_pool_entries != []:
            ...
            chik_config["pool"]["pool_list"] = []
            save_config(root_path, "config.yaml", chik_config)
