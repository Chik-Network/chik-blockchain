...
from chik_rs import AugSchemeMPL, G1Element, G2Element, ProofOfSpace
...
from chik.plotting.prover import PlotVersion, V1Prover, V2Prover, V2Quality
...
from chik.protocols.harvester_protocol import PartialProofsData, Plot, Plot2, PlotSyncResponse
...
class HarvesterAPI:
    ...
    async def _handle_v2_responses(
        self, v2_awaitables: Sequence[Awaitable[PartialProofsData | None]], start_time: float, peer: WSChikConnection
    ) -> int:
    ...
    async def new_signage_point_harvester(
        self, new_challenge: harvester_protocol.NewSignagePointHarvester2, peer: WSChikConnection
    ) -> None:
        """
    ...
    @metadata.request(peer_required=True)
    async def request_plots(self, _: harvester_protocol.RequestPlots, peer: WSChikConnection) -> Message:
