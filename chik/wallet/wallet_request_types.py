...
from chik_rs import Coin, CoinRecord, G1Element, G2Element, PrivateKey
from chik_rs.sized_bytes import bytes32
from chik_rs.sized_ints import uint8, uint16, uint32, uint64
from typing_extensions import Self

from chik.data_layer.data_layer_wallet import DataLayerSummary, Mirror
from chik.data_layer.singleton_record import SingletonRecord
from chik.pools.pool_wallet_info import NewPoolWalletInitialTargetState, PoolWalletInfo
...
from chik.util.streamable import Streamable, streamable, streamable_enum
from chik.wallet import wallet_coin_store
...
from chik.wallet.util.clvk_streamable import json_deserialize_with_clvk_streamable
...
from chik.wallet.util.wallet_types import StreamableWalletIdentifier, WalletType
...
class NFTMintBulk(TransactionEndpointRequest):
    ...
    xck_coins: list[Coin] | None = None
    xck_change_target: str | None = None
    ...
    def convert_to_proxy(self, proxy_type: type[_T_SendTransactionMultiProxy]) -> _T_SendTransactionMultiProxy:
        if proxy_type is CATSpend:
            if self.morph_bytes is not None:
                raise ValueError(
                    'Specified "morph_bytes" for a CAT-type wallet. Maybe you meant to specify an XCK wallet?'
                )
                ...
                raise ValueError(
                    ...
                    "Maybe you meant to specify an XCK wallet?"
                )
            ...
            if self.amount is not None:
                raise ValueError('Specified "amount" for an XCK wallet. Maybe you meant to specify a CAT-type wallet?')
            elif self.inner_address is not None:
                raise ValueError(
                    'Specified "inner_address" for an XCK wallet. Maybe you meant to specify a CAT-type wallet?'
                )
            elif self.memos is not None:
                raise ValueError('Specified "memos" for an XCK wallet. Maybe you meant to specify a CAT-type wallet?')
                ...
                raise ValueError('"additions" are required for XCK wallets.')
...
class _OfferEndpointResponse(TransactionEndpointResponse):
    ...
    @classmethod
    def from_json_dict(cls, json_dict: dict[str, Any]) -> Self:
        tx_endpoint: TransactionEndpointResponse = json_deserialize_with_clvk_streamable(
