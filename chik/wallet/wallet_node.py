...
import aiosqlite
from chik_rs import AugSchemeMPL, CoinState, ConsensusConstants, G1Element, G2Element, HeaderBlock, PrivateKey
...
from chik.util.db_wrapper import SQLITE_MAX_VARIABLE_NUMBER, manage_connection
from chik.util.errors import Err, KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
...
async def request_and_validate_header_block(
    peer: WSChikConnection, height: uint32, log: logging.Logger
) -> HeaderBlock | None:
...
class WalletNode:
    ...
    async def _send_transaction_message(
        self, peer: WSChikConnection, msg: Message, msg_name: bytes32 | None = None
    ) -> None:
    ...
    def initialize_wallet_peers(self) -> None:
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                dns_servers=self.config.get("dns_servers", ["dns-introducer.chiknetwork.com"]),
    ...
    async def _collect_valid_states(
        self, inner_states: list[CoinState], peer: WSChikConnection, cache: PeerRequestCache, fork_height: uint32 | None
    ) -> list[CoinState]:
    ...
    async def get_timestamp_for_height_from_peer(
        self, height: uint32, peer: WSChikConnection, expected_header_hash: bytes32 | None = None
    ) -> uint64 | None:
        """
    ...
    async def request_fee_estimates(self, peer: WSChikConnection, time_targets: list[uint64]) -> FeeEstimateGroup:
    ...
    async def wallet_short_sync_backtrack(
        self, peak_hb: HeaderBlock | None, header_block: HeaderBlock, peer: WSChikConnection
    ) -> int | None:
        top = header_block
        blocks = [top]
    ...
    async def get_coin_state(
        self, coin_names: list[bytes32], peer: WSChikConnection, fork_height: uint32 | None = None
    ) -> list[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSChikConnection, fork_height: uint32 | None = None
    ) -> list[CoinState]:
