...
def program_serialize_klvm_streamable(
    klvm_streamable: Streamable, translation_layer: TranslationLayer | None = None
) -> Program:
...
def program_deserialize_klvm_streamable(
    program: Program, klvm_streamable_type: type[_T_Streamable], translation_layer: TranslationLayer | None = None
) -> _T_Streamable:
...
def byte_deserialize_klvm_streamable(
    blob: bytes, klvm_streamable_type: type[_T_Streamable], translation_layer: TranslationLayer | None = None
) -> _T_Streamable:
...
#       useful for now
def is_klvm_streamable_type(v: type[object]) -> bool:
    return isinstance(v, type) and issubclass(v, Streamable) and hasattr(v, "_klvm_streamable")
...
def json_deserialize_with_klvm_streamable(
    ...
    if is_klvm_streamable_type(streamable_type) and isinstance(json_dict, str):
...
class TranslationLayer:
    ...
    ) -> TranslationLayerMapping[_T_KlvmStreamable, Streamable] | None:
