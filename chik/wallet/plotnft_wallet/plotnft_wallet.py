...
from chik_rs.chik_rs import Coin, G1Element
...
from chik.pools.pool_wallet_info import PoolSingletonState, PoolState, PoolWalletInfo
from chik.server.ws_connection import WSChikConnection
from chik.types.blockchain_format.program import Program
from chik.wallet.conditions import AssertCoinAnnouncement, Condition, CreateCoin, CreateCoinAnnouncement, Remark
from chik.wallet.puzzles.custody.custody_architecture import DelegatedPuzzleAndSolution
...
from chik.wallet.wallet_action_scope import PlotNFTTargetStateInfo, WalletActionScope
from chik.wallet.wallet_coin_record import WalletCoinRecord
from chik.wallet.wallet_info import WalletInfo
from chik.wallet.wallet_protocol import GSTOptionalArgs
...
class PlotNFT2Wallet:
    ...
    @classmethod
    async def create(
        cls, *, wallet_state_manager: WalletStateManager, xck_wallet: Wallet, wallet_info: WalletInfo
    ) -> Self:
        self = cls(
            wallet_state_manager=wallet_state_manager,
            xck_wallet=xck_wallet,
    ...
    async def create_new(
        ...
        *,
        ...
        origin_coins = await xck_wallet.select_coins(amount=uint64(fee + 1), action_scope=action_scope)
        announcement_assertions, coin_spends, new_plotnft = PlotNFT.launch(
            origin_coins=list(origin_coins),
            user_config=UserConfig(synthetic_pubkey=xck_wallet.convert_public_key_to_synthetic(target_pubkey)),
        ...
        await xck_wallet.generate_signed_transaction(
            amounts=[uint64(1)],
        ...
        coin_spends = plotnft.claim_pool_rewards(
            ...
            reward_delegated_puzzles_and_solutions=[
                DelegatedPuzzleAndSolution(
                    puzzle=self.xck_wallet.make_solution(
                        primaries=[
                            CreateCoin(
    ...
    async def join_pool(
        ...
        *,
        ...
        if fee > 0:
            await self.xck_wallet.create_tandem_xck_tx(
                fee=fee,
                action_scope=action_scope,
        ...
        try:
            ...
            if matched_plotnft_wallet_id is None and coin_spend.coin.parent_coin_info == next_plot_nft.launcher_id:
                ...
                wallet_state_manager.wallets[matched_plotnft_wallet_id] = await PlotNFT2Wallet.create(
                    wallet_state_manager=wallet_state_manager,
                    xck_wallet=wallet_state_manager.main_wallet,
                    wallet_info=WalletInfo(
