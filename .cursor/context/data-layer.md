...
- `DataStore` is the local Merkle/data persistence authority. It stores root generations/status in SQLite, stores Merkle blobs under `merkle_blobs_path`, stores or references key/value blobs under `key_value_blobs_path`, and delegates tree mutation/proof mechanics to `chik_rs.datalayer.MerkleBlob`.
...
- DataLayer wallet code depends on singleton CLVK structure, odd singleton amounts, lineage proofs, and offer solver field names. Changes in wallet puzzle drivers or offer summaries can break this module without direct edits here.
...
- Service coordination and RPC-facing behavior: `chik/data_layer/data_layer.py`, `chik/data_layer/data_layer_rpc_api.py`.
- Local Merkle/data persistence: `chik/data_layer/data_store.py`.
- Static file validation/download/upload: `chik/data_layer/download_data.py`, `chik/data_layer/data_layer_server.py`, `chik/data_layer/s3_plugin_service.py`.
- Wallet singleton authority: `chik/data_layer/data_layer_wallet.py`, `chik/data_layer/dl_wallet_store.py`.
- Offer/proof request contracts: `chik/data_layer/data_layer_util.py`, `chik/protocols/wallet_protocol.py`.
