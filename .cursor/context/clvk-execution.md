# CLVK / Puzzle Execution — Deep Context

> Attach when touching puzzle execution, spend validation, generator logic,
> condition processing, or `chik/types/blockchain_format/`.

## Key files

| File                                                 | Role                                                |
| ---------------------------------------------------- | --------------------------------------------------- |
| `chik/consensus/condition_tools.py`                  | `pkm_pairs()` — extract AGG_SIG conditions          |
| `chik/consensus/generator_tools.py`                  | `get_block_header()` — strip generator from block   |
| `chik/consensus/get_block_generator.py`              | `get_block_generator()` — resolve generator refs    |
| `chik/consensus/condition_costs.py`                  | Condition opcode costs                              |
| `chik/types/blockchain_format/program.py`            | CLVK program wrapper                                |
| `chik/types/blockchain_format/serialized_program.py` | Lazy deserialization                                |
| `chik/types/blockchain_format/coin.py`               | `Coin(parent_id, puzzle_hash, amount)`              |
| `chik/types/condition_opcodes.py`                    | All condition opcode values                         |
| `chik/types/condition_with_args.py`                  | `ConditionWithArgs`                                 |
| `chik/types/generator_types.py`                      | `BlockGenerator`, `NewBlockGenerator`               |
| `chik/types/clvk_cost.py`                            | Cost constants                                      |
| `chik/full_node/mempool_manager.py`                  | `pre_validate_spendbundle()`, `is_clvk_canonical()` |
| `chik/wallet/conditions.py`                          | Condition construction for wallet spends            |
| `chik/wallet/puzzles/`                               | CLVK puzzle source files                            |
...
- Cost limit: `MAX_BLOCK_COST_CLVK` (11 000 000 000)
...
Every CLVK operation has an associated cost. Total cost per block cannot
exceed `MAX_BLOCK_COST_CLVK = 11 000 000 000`.

### Generator byte cost

Each byte of generator program costs `COST_PER_BYTE = 12 000` CLVK cost
units, in addition to execution cost.

### Atom/pair bounds (mempool only)

After CLVK execution:

```python
if sbc.num_atoms > sbc.cost * 60_000_000 / MAX_BLOCK_COST_CLVK:
    reject  # too many atoms
if sbc.num_pairs > sbc.cost * 60_000_000 / MAX_BLOCK_COST_CLVK:
...
### `is_clvk_canonical(clvk_buffer)`

Checks that a CLVK program uses shortest-form atom encoding:
...
### `is_atom_canonical(clvk_buffer, offset)`

Validates a single atom's length prefix encoding. The CLVK format uses
...
Condition opcode byte values are a CLVK output ABI contract owned by `chik/types/condition_opcodes.py`; see `types.md` for the compatibility invariant. Read the enum directly for exact values rather than relying on a copied table, since renumbering or reusing a byte value changes consensus and wallet behavior.
...
When set during CLVK execution:
