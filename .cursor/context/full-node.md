...
| file                                    | owns                                                 |
| --------------------------------------- | ---------------------------------------------------- |
| `chik/full_node/full_node.py`           | peak processing, sync orchestration, peer/RPC fanout |
| `chik/full_node/full_node_api.py`       | peer message handlers, tx/block/SP intake            |
| `chik/full_node/mempool_manager.py`     | transaction-admission authority                      |
| `chik/full_node/full_node_store.py`     | volatile SP/EOS/unfinished-block caches              |
| `chik/full_node/block_store.py`         | full block + peak persistence, in_main_chain         |
| `chik/full_node/coin_store.py`          | current-peak UTXO view, FF lineage markers           |
| `chik/full_node/sync_store.py`          | peak-to-peer targets, sync flags                     |
| `chik/full_node/tx_processing_queue.py` | per-peer tx backpressure/DoS queue                   |
...
- RPC is semi-trusted but still crosses a boundary. Expensive endpoints run CLVK or block-generator work, read chain/mempool state, and sometimes take `priority_mutex` at low priority. Keep RPC reads consistent with the current main chain when returning spend/addition/removal state.
...
For exact store schemas, volatile cache behavior, sync-target handling, transaction backpressure, mempool admission behavior, and block-processing sequence, read the source files in `chik/full_node/` listed in the Landmarks table above. For focused coverage, start with the matching tests under `chik/_tests/core/full_node/stores/`, plus `chik/_tests/core/full_node/test_tx_processing_queue.py`.
