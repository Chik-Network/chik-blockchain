...
Scope: `chik/consensus/`. This is distilled architectural context for future audit or implementation agents. It intentionally omits file inventory and obvious helper summaries.
...
| file                                        | owns                                          |
| ------------------------------------------- | --------------------------------------------- |
| `chik/consensus/blockchain.py`              | add_block() commit authority, peak/fork state |
| `chik/consensus/block_body_validation.py`   | coin/tx state transition checks, ForkInfo use |
| `chik/consensus/block_header_validation.py` | proof/timing/slot/header checks               |
| `chik/consensus/augmented_chain.py`         | speculative overlay for batch validation      |
| `chik/consensus/blockchain_mmr.py`          | post-HF2 header MMR state                     |
| `chik/consensus/pot_iterations.py`          | iteration/difficulty math                     |
| `chik/consensus/default_constants.py`       | consensus constants                           |

## Implementation Authority

`chik.consensus` is the local authority for deciding whether a block is valid and whether it becomes the canonical peak. It does not own networking, sync orchestration, or wallet notifications; those live in full-node modules. It does own the consensus-facing view of chain state: block records, height-to-hash mapping, fork-local additions/removals, difficulty and sub-slot iteration transitions, VDF/POS validation, reward/fee/body checks, and post-HF2 header commitments.
...
- CLVK cost and canonical generator encoding after SF9;
...
- Rust CLVK generator execution and `SpendBundleConditions` production;
...
For files in the Landmarks table above, read the source in `chik/consensus/` directly. In-module but not landmarked: `chik/consensus/block_rewards.py` (reward/fee amount math). Cross-module authority: `chik/full_node/mempool_manager.py`. For regression coverage, start with `chik/_tests/blockchain/test_blockchain.py`, `chik/_tests/blockchain/test_augmented_chain.py`, `chik/_tests/blockchain/test_block_commitments.py`, and `chik/_tests/core/consensus/test_mmr.py`.
