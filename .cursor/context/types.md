...
Scope: `chik/types/`. This is distilled architectural context for future audit or implementation agents. It intentionally omits file inventory and obvious helper summaries.

## When To Read This

Read this for shared blockchain-format types, CLVK `Program` helpers, `Coin`/condition contracts, proof/VDF wrappers, block generator carriers, mempool item carriers, peer info types, and streamable schemas under `chik/types/`. For ownership of behavior, also read the consuming module context: consensus, full node, wallet, server, farmer/harvester, or timelord.

## Implementation Authority

`chik.types` is the shared type and helper boundary between consensus, full node, wallet, farmer/harvester, timelord, networking, RPC, and tests. It does not usually own orchestration or persistence. Its job is to expose stable Python-facing shapes for consensus objects, CLVK programs, block generators, mempool items, VDF/POS checks, peer addresses, fee units, condition opcodes, and lightweight protocol-adjacent records.

The most important design fact is that many "types" are Rust-backed `chik_rs` objects re-exported or wrapped for Python compatibility. Local Python code often adds only:

- compatibility imports for historical module paths;
- CLVK convenience APIs that convert between `Program` and `SerializedProgram`;
...
`chik_rs` owns the canonical implementations for core consensus types and CLVK execution-heavy objects: `Coin`, `CoinSpend`, `SpendBundle`, `SpendBundleConditions`, `ProofOfSpace`, `VDFInfo`, `VDFProof`, `BlockRecord`, `G1Element`, `G2Element`, and `SerializedProgram` (`chik_rs.Program`). Python files in this module frequently preserve old import paths while delegating behavior to Rust.

The boundary is compatibility-sensitive:

- `SerializedProgram` is an alias to `chik_rs.Program`, not a Python class. `isinstance(..., SerializedProgram)` checks are really checks against the Rust extension type.
...
## CLVK Program Contracts

`blockchain_format.program.Program` is the Python CLVK s-expression wrapper used heavily by wallet puzzle construction, tests, simulator tooling, and some RPC paths. It is not the consensus-hot block validation path, but it calls Rust CLVK execution and tree hashing.

Key contracts:

- `Program.from_bytes()` intentionally parses through the Rust CLVK path. This gives a Python-compatible object while using the faster Rust parser/LazyNode path.
...
- `curry()` and `uncurry()` encode/decode the canonical CLVK curry shape. Wallet puzzle drivers and tests assume this shape when matching layered puzzles.
- `sha256_treehash()` is deliberately iterative to avoid Python recursion limits on deeply nested CLVK. Replacing it with a recursive implementation changes a robustness property.
...
- `coin_as_list()` preserves the CLVK list order `[parent_coin_info, puzzle_hash, amount]`. This order feeds coin-name and puzzle semantics across wallet and consensus code.
...
- `ConditionOpcode` byte values are CLVK output ABI. Renaming is low risk; changing numeric bytes or deleting legacy opcodes is consensus/protocol risk.
...
- `NewBlockGenerator.cost` is the total CLVK plus byte plus condition cost used by block creation. Do not confuse it with per-spend cost or mempool virtual cost.
...
- `chik.consensus` consumes `BlockGenerator`, `ValidationState`, VDF/POS helpers, `Coin`, and generator/program aliases during block validation, difficulty/slot checks, and fork-aware generator resolution.
- `chik.full_node` consumes `MempoolItem`, `NewBlockGenerator`, `BlockGenerator`, `PeerInfo`, VDF/POS helpers, and `WeightProof` for sync, block creation, tx admission, RPC, and peer handling.
- `chik.wallet` consumes `Program`, `SerializedProgram`, `make_spend()`, condition opcodes, signing modes, weight-proof carriers, and peer info for puzzle construction, signing, SPV sync, and tests.
- `chik.server` and discovery consume peer-info types for connection identity context, address-book bucketing, and RPC-facing peer data.
- `chik.farmer`, `chik.harvester`, `chik.plotting`, and `chik.timelord` consume proof/VDF helpers and plot-key derivation functions around signage points, VDF validation, and plot creation.

This module generally should not import upward into those consumers. Its dependency direction is toward `chik_rs`, `clvk`, `chikpos`, `chikvdf`, and low-level `chik.util` helpers.
...
- `FeeRate.create()` rounds up with `math.ceil()`. This prevents underquoting when converting mojos over CLVK cost and mirrors protocol fee-estimate compatibility expectations.
- `Mojos` and `CLVKCost` are `NewType` wrappers around `uint64`; they add static clarity only. Runtime code still receives integers.
...
- Do not conflate `Program` and `SerializedProgram`: wallet construction often needs Python CLVK traversal, while full-node validation and generator paths expect serialized Rust-backed bytes.
...
For exact type definitions and helper behavior, read the owning files under `chik/types/`, especially `chik/types/blockchain_format/`, `chik/types/blockchain_format/proof_of_space.py`, `chik/types/condition_opcodes.py`, `chik/types/condition_with_args.py`, `chik/types/generator_types.py`, `chik/types/mempool_item.py`, and `chik/types/peer_info.py`. For proof-of-space behavior changes, also inspect `chik/_tests/core/custom_types/test_proof_of_space.py`.
