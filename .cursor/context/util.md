...
| file                             | owns                                        |
| -------------------------------- | ------------------------------------------- |
| `chik/util/streamable.py`        | deterministic serialization, get_hash()     |
| `chik/util/db_wrapper.py`        | SQLite writer/reader transaction semantics  |
| `chik/util/keychain.py`          | mnemonic/public/private-key operations      |
| `chik/util/keyring_wrapper.py`   | keyring backend selection, passphrase cache |
| `chik/util/config.py`            | YAML config load/save hygiene               |
| `chik/util/priority_mutex.py`    | priority-ordered lock (block > tx)          |
| `chik/util/limited_semaphore.py` | bounded active+waiting DoS backpressure     |
| `chik/util/lru_cache.py`         | LRU/FIFO bounded cache primitives           |
...
- `config.py`, `lock.py`, `files.py`, and `path.py` are the persistence hygiene
  ...
  relative paths under the Chik root.
...
- The file keyring's on-disk encryption format and the default (no-user-passphrase) passphrase are compatibility-sensitive: changing either strands existing keyrings without a migration. Exact KDF/cipher/file-layout details live in `chik/util/file_keyring.py`.
...
- Changing `streamable` parse/stream/coercion behavior without checking protocol,
  ...
  larger than `chik/util/streamable.py`.
...
- `chik/_tests/core/util/test_streamable.py` covers decorator, binary, JSON,
  list-limit, enum, and type-conversion behavior for streamables.
- `chik/_tests/core/test_db_validation.py`, `chik/_tests/core/test_db_conversion.py`,
  store tests under `chik/_tests/core/full_node/stores/`, wallet RPC tests, and
  data-layer tests exercise `DBWrapper2` under realistic store usage.
- `chik/_tests/util/test_lru_cache.py` documents bounded keyed-list cache
  semantics.
- `chik/_tests/core/full_node/test_full_node_api_rate_hardening.py` and full-node
...
For exact infrastructure behavior, read the owning utility in `chik/util/` rather than copied notes; the highest-impact primitives are listed in the Landmarks table above. Related non-landmarked files: `chik/util/file_keyring.py` (encrypted key file format) and `chik/util/errors.py` (`Err` enum).
