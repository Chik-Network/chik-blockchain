...
| file                                | owns                                                  |
| ----------------------------------- | ----------------------------------------------------- |
| `chik/server/ws_connection.py`      | per-peer protocol, request/response, rate-limit state |
| `chik/server/server.py`             | connection admission, bans, TLS identity, broadcast   |
| `chik/server/rate_limits.py`        | v1/v2 per-message + aggregate accounting              |
| `chik/server/rate_limits_v3.py`     | in-flight-window v3 limiter                           |
| `chik/server/rate_limit_numbers.py` | rate-limit tables/constants                           |
| `chik/server/address_manager.py`    | peer address-book state machine                       |
| `chik/server/node_discovery.py`     | DNS/introducer discovery orchestration                |

## Implementation Authority

- `WSChikConnection` is the per-peer protocol authority. It owns handshake-derived peer type/capabilities, request nonce allocation, pending-response matching, v2/v3 rate-limit state, API task lifetime, and raw websocket parsing.
- `ChikServer` is the process-local connection authority. It owns TLS context selection, node identity, inbound/outbound admission, duplicate/self connection rejection, bans, broadcast validation, and connection shutdown callbacks.
...
- `call_api()` validates the response type against `protocol_state_machine.VALID_REPLY_MESSAGE_MAP` and bans on mismatches. Fire-and-forget broadcasts must not use request/response message types; `ChikServer.validate_broadcast_message_type()` treats that as an internal protocol error and closes relevant peers.
...
- `ChikServer.connection_closed()` refuses to ban localhost, trusted peers, and exempt networks even when a lower layer requested a ban. Audit ban behavior at the server callback, not at the call site that passed `ban_time`.
...
- `Service.manage()` shutdown order is intentional: release UPnP in the background, cancel reconnect loop, close peer connections, close RPC, await `ChikServer`, then await RPC. Reordering can leave reconnect or RPC paths racing against closed peer state.
- `chik_policy.set_chik_policy()` mutates the process event-loop policy and wraps server creation with `PausableServer`. Connection limits are enforced at accept/pause level, separately from ChikServer's node-type inbound slot limits.
...
- Public IP discovery in `ChikServer.get_peer_info()` depends on external HTTPS services and returns `None` on failure. Callers must handle absence of self-advertisable peer info without degrading core connectivity.
...
For exact websocket dispatch, peer-close behavior, rate-limit tables, and discovery/address-book rules, read the source files in `chik/server/` listed in the Landmarks table above.
