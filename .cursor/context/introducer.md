# Chik Introducer Module Context

Verified: 2026-07-12 against 24db9ad3901d. If source contradicts this doc, trust source and update the doc.

`chik/introducer/` is a small bootstrap service whose job is to help wallets,
...
- `ChikServer`, not `Introducer`, owns the in-memory peer set. It creates
...
- Vetting is a TCP reachability check only: the service periodically samples
  ...
  timeout, then closes it. It does not perform a Chik handshake or verify node
  type during vetting.
...
- If `introducer.dns_servers` is omitted or empty on mainnet, startup injects
  `dns-introducer.chiknetwork.com`. Other networks depend on explicit config.
...
- The service uses public SSL paths from the full-node certificate config and
  normal ChikServer handshake/rate-limit machinery. Introducer-specific behavior
...
- `on_connect()` and state-change callbacks are intentionally no-ops today. The
  meaningful connect side effect is in `ChikServer.incoming_connection()`, which
...
- Avoid treating `vetted` as proof of a valid Chik full node. It only proves a
...
- Introducer service state and vetting: `chik/introducer/introducer.py`.
- Peer request/reply handling: `chik/introducer/introducer_api.py`.
- Service startup and wiring: `chik/introducer/start_introducer.py`, `chik/introducer/introducer_service.py`.
