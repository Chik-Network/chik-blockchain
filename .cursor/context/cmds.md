...
- `chik.py` is the root Click command tree. It creates `ChikCliContext`, records
...
- `ChikCliContext` is the in-process CLI context authority. Commands read root
...
- `cmd_classes.py` is the newer dataclass-command DSL. `@chik_command` freezes a
  command class, turns `option()` fields into Click options, recursively expands
  `@command_helper` fields, injects `ChikCliContext` only for fields named
...
- `start_funcs.py` and `stop.py` cross into the daemon websocket authority. The
  CLI launches `chik run_daemon --wait-for-unlock`, unlocks the daemon keyring
  when needed, and starts/stops service names resolved from
  `chik.util.service_groups`.
...
- Top-level options are applied before subcommand execution. Any code that needs
  ...
  `ChikCliContext.set_default(ctx)` rather than re-resolving defaults.
...
- `param_types.py` is the CLI conversion authority for fees, wallet amounts,
  addresses, `bytes32`, and `uint64`. Fees are decimal XCK strings capped at
  a source-defined XCK cap and converted to mojos; wallet amounts are stored as `CliAmount`
  until the target wallet unit is known.
- `AddressParamType` validates the selected network prefix for XCK/TXCK
  addresses by consulting `ChikCliContext.expected_prefix` or loading
  `config.yaml`. Non-XCK address families are mapped through `AddressType`.
...
- Address-prefix caching in `ChikCliContext` is per Click invocation. Changes
...
- CLI tests usually use `click.testing.CliRunner`, temporary Chik roots, and
  monkeypatched RPC client factories from `chik/_tests/cmds/cmd_test_utils.py`.
...
- Root CLI and command context: `chik/cmds/chik.py`, `chik/cmds/cmd_classes.py`, `chik/cmds/cmd_helpers.py`.
- Common RPC/config/keychain helpers: `chik/cmds/cmds_util.py`, `chik/cmds/init_funcs.py`, `chik/cmds/configure.py`.
- Wallet and transaction command surfaces: `chik/cmds/wallet.py`, `chik/cmds/wallet_funcs.py`, `chik/cmds/coin_funcs.py`, `chik/cmds/param_types.py`.
- Service control commands: `chik/cmds/start_funcs.py`, `chik/cmds/stop.py`, `chik/cmds/rpc.py`.
