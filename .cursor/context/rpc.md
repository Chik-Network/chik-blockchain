...
- `RpcServiceProtocol` is the service adapter contract. The service supplies its `ChikServer`, connection summaries, outbound peer connection hook, state-change callback registration, and optional lifecycle management.
...
- Do not assume `marshal()` covers every typed or CLVK-streamable request path.
...
- `get_connections()` crosses into `ChikServer` through the concrete service. Returned connection data includes peer node id bytes; clients convert `node_id` hex strings back to bytes.
- `open_connection()` resolves hostnames with the configured IPv6 preference and delegates to `ChikServer.start_client()` with the service's `on_connect` callback when present.
...
- Default marshalling uses `from_json_dict()` and `to_json_dict()`. Requests carrying the CLVK-streamable JSON CHIP use CLVK streamable JSON serialization, optionally with a named translation layer.
...
- Wallet RPC also performs CLVK-streamable JSON handling in transaction wrappers. Do not assume `marshal()` is the only CLVK-streamable JSON path in the RPC system.
...
- Shared RPC changes should run focused tests under `chik/_tests/rpc/` plus any service-specific RPC tests for affected semantics. Use `tools/pytest`, not bare `pytest`.
...
- For marshalling changes, test normal streamable JSON and CLVK-streamable requests. Data-layer and wallet RPC are the important service-specific consumers.
...
- Shared server lifecycle and common routes: `chik/rpc/rpc_server.py`.
- Common client behavior: `chik/rpc/rpc_client.py`.
- Typed marshalling and CLVK streamable handling: `chik/rpc/util.py`.
- Structured errors: `chik/rpc/rpc_errors.py`.
- Daemon websocket envelope/routing: `chik/util/ws_message.py`, `chik/daemon/server.py`.
