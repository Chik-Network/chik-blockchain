# Chik Solver Module Context

Verified: 2026-07-12 against 24db9ad3901d. If source contradicts this doc, trust source and update the doc.

`chik/solver/` is a small service boundary for v2 plot solving. It receives `SolverInfo` messages from farmers, turns Rust-backed `PartialProof` fragments into full proof bytes via `chik_rs.solve_proof()`, and returns `SolverResponse` messages that the farmer folds back into normal proof-of-space processing. It is not a chain, plot, key, or farmer-state authority.
...
- `Solver` owns only process-local runtime state: root/config, service lifecycle flags, a `ThreadPoolExecutor`, consensus constants, and the attached `ChikServer`.
...
- Do not add large solver payloads without revisiting protocol and rate-limit ownership in `chik/protocols/` and `chik/server/`.
...
- `start_solver.create_solver_service()` follows the standard Chik service composition: load selected-network constants, build `Solver`, `SolverAPI`, optional RPC, then wrap in `Service` as `NodeType.SOLVER`.
...
- `chik/farmer/` is the only meaningful runtime consumer. Farmer RPC also exposes `connect_to_solver`, which closes existing solver connections before opening a requested solver peer.
- `chik/harvester/` drives solver demand indirectly by emitting v2 `PartialProofsData`; v1 plots bypass the solver entirely and return full proofs directly.
- `chik/protocols/` owns `SolverInfo`, `SolverResponse`, message IDs, sender authorization, and shared protocol versioning.
- `chik/server/` owns TLS identity, handshake-derived node type, rate limits, peer filtering, broadcast validation, and API dispatch. Solver code should not bypass these gates.
- `chik_rs` owns the actual proof-solving algorithm. Python solver behavior is mostly orchestration, error handling, and boundary enforcement around `solve_proof()`.
...
- Solver service/API/RPC: `chik/solver/solver.py`, `chik/solver/solver_api.py`, `chik/solver/solver_rpc_api.py`, `chik/solver/start_solver.py`.
- Farmer-side request correlation: `chik/farmer/farmer.py`, `chik/farmer/farmer_api.py`.
- Harvester partial-proof production: `chik/harvester/harvester_api.py`, `chik/plotting/prover.py`.
- Wire contracts and sender authorization: `chik/protocols/solver_protocol.py`, `chik/protocols/protocol_message_types.py`, `chik/protocols/protocol_state_machine.py`.
- Proof expansion implementation boundary: `chik_rs.solve_proof`.
