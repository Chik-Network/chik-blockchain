...
| file                            | owns                                                  |
| ------------------------------- | ----------------------------------------------------- |
| `chik/farmer/farmer.py`         | mutable service state, SP/proof/pool/plot-sync caches |
| `chik/farmer/farmer_api.py`     | SP/proof/signature/solver message routing             |
| `chik/farmer/farmer_rpc_api.py` | reward target, pool state, harvester plot RPC         |
| `chik/farmer/start_farmer.py`   | farmer Service construction                           |
...
- Pooling configuration lives in `<chik root>/pooling/pooling_share_state.yaml`, guarded by `PoolingShareState.lock()`. `Farmer.__init__()` performs migration from older `config.yaml` pool entries.
...
- Pool config mirror and external pool protocol: `chik/pools/pool_config.py`, `chik/protocols/pool_protocol.py`.
- Harvester/solver wire contracts: `chik/protocols/harvester_protocol.py`, `chik/protocols/solver_protocol.py`.
