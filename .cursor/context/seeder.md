# Chik Seeder Module Context

Verified: 2026-07-12 against 24db9ad3901d. If source contradicts this doc, trust source and update the doc.

`chik/seeder/` is two cooperating services over one SQLite database: a crawler
that joins the Chik P2P network as a minimal full node to discover reachable
...
- `CrawlerAPI` is a deliberately stubby full-node API surface. It registers many
  full-node and wallet message handlers so generic `ChikServer` protocol
...
- Generic `Service` / `ChikServer` still own TLS identity, handshakes, node-type
...
- The crawler opens a bounded number of concurrent outbound client tasks. On connect it
  records the peer's short Chik version, requests `RespondPeers`, waits briefly
...
- Exact response limits, EDNS0 handling, and truncation behavior live in `chik/seeder/dns_server.py`.
...
- The crawler currently clears `server.banned_peers` after each batch. Changes to
  ...
  weakening generic `ChikServer` behavior elsewhere.
...
- Crawler service state and peer scoring: `chik/seeder/crawler.py`, `chik/seeder/crawl_store.py`, `chik/seeder/peer_record.py`.
- Crawler peer API and RPC: `chik/seeder/crawler_api.py`, `chik/seeder/crawler_rpc_api.py`.
- DNS server behavior: `chik/seeder/dns_server.py`.
- Service startup: `chik/seeder/start_crawler.py`, `chik/seeder/crawler_service.py`.
