...
| file                                  | owns                                                |
| ------------------------------------- | --------------------------------------------------- |
| `chik/wallet/wallet_state_manager.py` | central wallet state, coin/tx records, side effects |
| `chik/wallet/wallet_node.py`          | SPV peer sync, trusted/untrusted proof validation   |
| `chik/wallet/wallet_action_scope.py`  | transaction side-effect staging boundary            |
| `chik/wallet/wallet.py`               | standard spend construction                         |
| `chik/wallet/wallet_rpc_api.py`       | tx endpoint gating, TXConfig autofill               |
| `chik/wallet/coin_selection.py`       | coin selection accounting/exclusions                |
| `chik/wallet/outer_puzzles.py`        | puzzle-driver registry for layered assets           |
| `chik/wallet/trade_manager.py`        | offer/trade lifecycle, interested subscriptions     |
...
- `Offer.fees()` is accounting metadata, not a validation authority. Spend validity still comes from CLVK execution, announcement assertions, and wallet/full-node admission paths.
...
- Many asset recognizers and drivers depend on positional CLVK structure, dynamic `PuzzleInfo`/`Solver` fields, and `assert`-style invariants. Review parser changes with actual parent-spend and layer-order examples.
...
For coin-selection, sync, RPC, action-scope, puzzle-driver, and trade-manager behavior, read the files in the Landmarks table above. For persistence, timelock, and offer boundaries not landmarked, read `chik/wallet/wallet_transaction_store.py`, `chik/wallet/wallet_interested_store.py`, `chik/wallet/conditions.py`, `chik/wallet/trading/offer.py`, and `chik/wallet/trading/trade_store.py`.
