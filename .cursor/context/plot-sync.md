...
- Protocol payloads live in `chik/protocols/harvester_protocol.py`; `chik/plot_sync/` owns sequencing, delta semantics, and ACK/error handling. Schema edits are network compatibility changes and must move with protocol message IDs, node-type sender maps, rate limits, API decorators, and tests.
...
- Harvester `harvester_handshake` installs farmer/pool keys in `PlotManager`, binds `Sender` to the farmer `WSChikConnection`, starts the sender task, and then starts plot refreshing. This ordering matters: refresh before handshake can load plots without the farmer-owned key filter.
...
- Harvester sender state machine: `chik/plot_sync/sender.py`.
- Farmer receiver state machine: `chik/plot_sync/receiver.py`.
- Plot-sync payloads: `chik/protocols/harvester_protocol.py`.
- Farmer receiver ownership and RPC consumers: `chik/farmer/farmer.py`, `chik/farmer/farmer_rpc_api.py`.
- Harvester refresh callbacks that feed sync: `chik/harvester/harvester.py`, `chik/plotting/manager.py`.
