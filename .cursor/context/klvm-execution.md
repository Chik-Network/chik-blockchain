# KLVM / Puzzle Execution — Deep Context
...
| `chik/consensus/cost_calculator.py`                  | `NPCResult` — name/puzzle/conditions result         |
| `chik/consensus/condition_costs.py`                  | Condition opcode costs                              |
| `chik/types/blockchain_format/program.py`            | KLVM program wrapper                                |
...
| `chik/types/klvm_cost.py`                            | Cost constants                                      |
| `chik/full_node/mempool_manager.py`                  | `pre_validate_spendbundle()`, `is_klvm_canonical()` |
| `chik/wallet/conditions.py`                          | Condition construction for wallet spends            |
| `chik/wallet/puzzles/`                               | KLVM puzzle source files                            |
...
- Cost limit: `MAX_BLOCK_COST_KLVM` (11 000 000 000)
...
Every KLVM operation has an associated cost. Total cost per block cannot
exceed `MAX_BLOCK_COST_KLVM = 11 000 000 000`.

### Generator byte cost

Each byte of generator program costs `COST_PER_BYTE = 12 000` KLVM cost
units, in addition to execution cost.

### Atom/pair bounds (mempool only)

After KLVM execution:

```python
if sbc.num_atoms > sbc.cost * 60_000_000 / MAX_BLOCK_COST_KLVM:
    reject  # too many atoms
if sbc.num_pairs > sbc.cost * 60_000_000 / MAX_BLOCK_COST_KLVM:
...
### `is_klvm_canonical(klvm_buffer)`

Checks that a KLVM program uses shortest-form atom encoding:
...
### `is_atom_canonical(klvm_buffer, offset)`

Validates a single atom's length prefix encoding. The KLVM format uses
...
**Location**: `chik/types/condition_opcodes.py`

Key conditions (from KLVM spend output):
...
When set during KLVM execution:
