...
> Attach when touching `chik/server/`, `chik/protocols/`, connection handling,
...
| File                       | Lines | Role                                                 |
| -------------------------- | ----- | ---------------------------------------------------- |
| `server.py`                | ~900  | `ChikServer`: connection management, message routing |
| `ws_connection.py`         | ~790  | `WSChikConnection`: single peer connection           |
...
| `chik_policy.py`           | ~370  | Custom asyncio event loop policy, connection limits  |
...
## `WSChikConnection` — Per-peer connection
...
- `peer_required=True` — handler receives `WSChikConnection` as parameter
...
**Location**: `server/chik_policy.py`

- Default `global_max_concurrent_connections = 250`
- `set_chik_policy(connection_limit)` sets effective limit to `connection_limit + 100`
- Custom event loop policy (`ChikProactorEventLoop` on Windows,
