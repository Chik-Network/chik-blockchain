...
Scope: `chik/_tests/util/`. This is distilled architectural context for future audit or implementation agents. It intentionally omits helper-by-helper inventory and focuses on contracts that affect the wider test suite.
...
`chik/_tests/util/` is the shared test infrastructure layer for the repository. It is not a leaf test directory: changes here can alter the meaning, runtime, or flake profile of blockchain, full-node, mempool, wallet, server, farmer/harvester, timelord, data-layer, plotting, and protocol tests.
...
- lightweight spend/mempool simulation for CLVK and wallet contract tests that need production mempool and coin-store behavior without a full node;
...
Treat edits here as harness changes, not local cleanup, unless the helper is demonstrably unused outside `chik/_tests/util/` itself.
...
- `chik/_tests/conftest.py` imports this module heavily. Session fixtures install the plot cache, isolate keyring access, create shared `BlockTools`, construct persistent chains by consensus mode, and expose simulator/wallet/full-node fixtures through these helpers.
...
- Mempool tests rely on `spend_sim.py`, `get_name_puzzle_conditions.py`, and `misc.invariant_check_mempool()` for production CLVK flags, spend-bundle validation, pool accounting, and fast-forward singleton invariants.
...
- Farmer/harvester and plotting tests rely on `setup_farmer_solver_multi_harvester()` and `plot_cache.install()`. The plot cache monkeypatches prover methods and `chik_rs.solve_proof` globally for the test process, merging cache state on exit under a file lock.
...
- `spend_sim.py` or `get_name_puzzle_conditions.py`: the targeted mempool/CLVK/wallet contract tests;
- `time_out_assert.py` or benchmark telemetry: a direct util test plus `chik/_tests/process_junit.py`-related expectations if telemetry shape changes;
...
- Service topology and environment setup: `chik/_tests/util/setup_nodes.py`, `chik/_tests/environments/`.
- Deterministic chain and DB helpers: `chik/_tests/util/blockchain.py`.
- Spend simulation and condition helpers: `chik/_tests/util/spend_sim.py`, `chik/_tests/util/get_name_puzzle_conditions.py`.
- Async and protocol helpers: `chik/_tests/util/time_out_assert.py`, `chik/_tests/util/network_protocol_data.py`.
- Plot and process helpers: `chik/_tests/util/plot_cache.py`, `chik/_tests/process_junit.py`.
