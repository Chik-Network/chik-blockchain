# chik-tests-blockchain

Verified: 2026-07-12 against 24db9ad3901d. If source contradicts this doc, trust source and update the doc.

Scope: `chik/_tests/blockchain/`. This is distilled architectural context for future audit or implementation agents. It intentionally omits test inventory and obvious fixture descriptions.
...
- block body validation, CLVK condition handling, and coin-store transitions;
...
`skip_prevalidation=True` is deliberate in malformed-body tests where the mutation would fail before the body code under test. In that mode, the helper fabricates `PreValidationResult` and signature conditions, so the test is no longer exercising header/CLVK prevalidation.
...
- Consensus regression tests: `chik/_tests/blockchain/`.
- Shared blockchain test helpers: `chik/_tests/blockchain/blockchain_test_utils.py`.
- Chain fixture generation: `chik/_tests/blockchain/test_build_chains.py`, `chik/_tests/util/blockchain.py`.
