# Chik Commands Tests Module Context

Verified: 2026-07-12 against 24db9ad3901d. If source contradicts this doc, trust source and update the doc.

`chik/_tests/cmds/` is the CLI contract test module. It primarily verifies that Click parsing, command helper dataclasses, config/root-path handling, RPC-client selection, wallet request construction, transaction output files, and user-facing text stay aligned with `chik/cmds/`. Most tests intentionally stop at the command/RPC boundary; they are not full wallet or node behavior tests unless they explicitly use service fixtures.
...
- `conftest.py` provides `get_test_cli_clients`, a module-scoped temporary Chik root with default config plus monkeypatches that replace `get_any_service_client`, `get_wallet_client`, and `cli_confirm` in both `chik.cmds.cmds_util` and the command modules that imported those functions directly. Tests using this fixture exercise the real CLI entrypoint while avoiding daemon/keychain/RPC processes.
...
- `run_cli_command()` mutates `sys.argv` and invokes the top-level `chik` CLI with `--root-path`. It treats any non-zero `SystemExit` as failure and returns captured stdout. This means tests often fail through `AssertionError` wrapping Click output rather than directly through Click exceptions.
...
- `test_cmd_framework.py` is the specification for the newer class-based command system in `chik.cmds.cmd_classes` and `chik.cmds.cmd_helpers`. It verifies dataclass command wrapping, sync/async `run()` dispatch, type-hint-to-Click option generation, nested `command_helper` parsing, context injection, and rejection of unsupported/default-incompatible type shapes.
...
- Many assertions compare rich dataclass instances from `chik.wallet.wallet_request_types`. A failing equality usually means a CLI argument no longer maps to the same public RPC contract, not just an output formatting change.

## Other Command Areas

- `test_show.py` uses fake full-node RPC responses plus synthetic `FullBlock`/`TestBlockRecord` objects to check `chik show` output and RPC call ordering for chain state, fee estimates, height lookup, and block printing.
...
- `wallet/test_wallet_check.py` is pure logic coverage for `chik.cmds.check_wallet_db` gap/contiguous-used-address checks and wallet type parsing from DB rows.
...
- Keep fee and amount expectations explicit about units. `TransactionFeeParamType` applies a source-defined decimal-XCK fee cap, while generic `AmountParamType` produces `CliAmount` and converts later using wallet-specific mojo-per-unit.
...
- Direct imports of patch targets in command modules are easy to miss. If a command file does `from chik.cmds.cmds_util import get_wallet_client`, patching only `cmds_util` will not affect that module's local binding.
...
- `ChikCliContext.expected_prefix` caches address prefix during parameter conversion. Address tests that pass without config may be depending on this cache; be careful when moving address parsing earlier or outside Click context.
...
- CLI test helpers and fixtures: `chik/_tests/cmds/cmd_test_utils.py`, `chik/_tests/cmds/conftest.py`.
- Command framework coverage: `chik/_tests/cmds/test_cmd_framework.py`, `chik/_tests/cmds/test_cmds_util.py`, `chik/_tests/cmds/test_click_types.py`.
- CLI implementation context: `chik/cmds/`.
