...
Scope: `chik/_tests/`. This is distilled architectural context for future audit or implementation agents. It focuses on the shared test harness and cross-suite contracts rather than inventorying individual test files.
...
When changing tests under this root, treat fixture edits as architecture changes. A small change in `chik/_tests/conftest.py`, `chik/_tests/util/setup_nodes.py`, `chik/_tests/connection_utils.py`, or wallet/data-layer conftests can alter the semantics of hundreds of tests.
...
This parametrization is one of the suite's strongest safeguards. It catches assumptions that only hold before or after a fork, especially around CLVK flags, canonical generator rules, plot eligibility, header commitments, transaction-block context, and cached block fixtures.
...
`chik/_tests/util/blockchain.py:create_blockchain()` creates a minimal in-memory production `Blockchain` with real `CoinStore`, `BlockStore`, `BlockHeightMap`, and `InlineExecutor`. It is the right base for isolated consensus/store tests where full service setup would obscure the invariant.

`chik/_tests/util/setup_nodes.py` is the service-topology layer. It builds real `Service` instances for full nodes, simulator nodes, wallets, farmers, harvesters, timelords, introducers, crawlers, seeders, daemons, and solvers. These are not mocks; tests using them are crossing process, RPC, websocket, DB, or peer-protocol boundaries.
...
`cmds/` tests usually patch RPC client factories through `get_test_cli_clients` and a temp config root. They are command-surface tests, not service integration tests, unless they explicitly use the root `chik_root` subprocess helpers.

`util/`, `clvk/`, `generator/`, `fee_estimation/`, `db/`, and custom-type tests are mostly fast deterministic guards for serialization, CLVK execution, cost accounting, DB wrappers, cache behavior, network protocol files, and helper APIs. Prefer local assertions here instead of service topology.
...
- Pool tests span config/CLI parsing, CLVK pool puzzle lifecycle, wallet pool store, `plotnft` commands, singleton identity, trusted/untrusted wallet sync, and reorg/revert.
...
For workflow-layout changes, update generated workflows as described by `chik/_tests/README.md`. For persistent block fixture changes, include the relevant `test_build_chains.py` coverage and expect artifact/runtime impact.

## Source Pointers

- Root fixtures and test configuration: `chik/_tests/conftest.py`, `chik/_tests/README.md`, `chik/_tests/testconfig.py`.
- Shared topology and utility layers: `chik/_tests/util/setup_nodes.py`, `chik/_tests/util/blockchain.py`, `chik/_tests/connection_utils.py`.
- Wallet environment harness: `chik/_tests/environments/wallet.py`, `chik/_tests/wallet/conftest.py`.
- CI matrix generation: `chik/_tests/build-job-matrix.py`.
