# Chik Plotting Module Context

Verified: 2026-07-12 against 24db9ad3901d. If source contradicts this doc, trust source and update the doc.

`chik/plotting/` is the shared plot lifecycle boundary. It creates plot files, opens and abstracts V1/V2 provers, scans configured harvester plot directories, persists a prover cache, parses plot memos into farming keys, and supplies the lock-protected plot inventory consumed by the harvester, plot sync, `chik plots check`, simulator block tools, and external plotter wrappers.
...
Read this for plot creation, plot IDs, memo parsing, V1/V2 prover abstraction, plot cache/refresh behavior, plot directory config helpers, and diagnostics such as `chik plots check`. For live signage-point handling, read `harvester.md`; for pool singleton state, read `pools.md`.

## Implementation Authority

- `create_plots.py` owns in-process chikpos V1 plot creation and experimental Rust-backed V2 plot creation. It resolves farmer/pool keys from explicit CLI values or the keychain, writes plot memo bytes, computes plot IDs, and calls `DiskPlotter` or `create_v2_plot()`.
...
- `prover.py` is the compatibility layer over external proof libraries. It hides V1 `chikpos.DiskProver` and V2 `chik_rs.Prover` behind `ProverProtocol`, preserving common calls for plot ID, memo, parameter, compression level, serialized prover data, and quality lookup.
...
- Do not infer production harvesting behavior from `chik plots check`; diagnostics intentionally open and inspect cases normal harvesting may reject.
...
- The plot public key is derived as `local_pk + farmer_pk` for pool-public-key plots, and `local_pk + farmer_pk + taproot_pk` for pool-contract plots. This must stay aligned with `chik.types.blockchain_format.proof_of_space.generate_plot_public_key()` and farmer aggregate-signature verification.
...
- Compressed plot handling crosses the `chikpos.decompressor_context_queue` boundary. GPU initialization may fall back to CPU harvesting; runtime decompressor failures may surface as stringly `RuntimeError`s from lower libraries.
...
- `Cache.load()` has a compatibility guard for suspicious oversized V1 prover data from older bladebit/chikpos behavior. Removing it can resurrect bad cache entries for users who have not manually deleted `plot_manager_v2.dat`.
...
- V1 uses `chikpos.DiskProver`; full proofs are retrieved locally with `get_full_proof(challenge, index, parallel_read)`, and proof validation uses `chikpos.Verifier`.
- V2 uses `chik_rs.Prover`; quality lookup returns `PartialProof` wrappers, and full proof solving is done by `chik_rs.solve_proof()` in diagnostics or by solver services in the farmer flow.
...
- Plot sync depends on `PlotRefreshResult.loaded`, `removed`, `failed_to_open_filenames`, `no_key_filenames`, and duplicate state. A local refresh change can become a farmer/UI compatibility change through `chik.plot_sync`.
...
- `chik plots create` and `chik.plotters.chikpos` both call `resolve_plot_keys()` and `create_plots()`; external plotter wrappers for bladebit and madmax call `resolve_plot_keys()` and pass the resulting keys to subprocess CLIs.
...
- `chik plots check` deliberately opens no-key plots, can list duplicate filename-style plot IDs, and can reveal raw memo bytes. Treat it as an admin/debug surface with more permissive loading than normal harvesting.
...
- External libraries (`chikpos`, `chik_rs`) define much of the runtime behavior and exception surface. Tests should not assume all invalid plots or decompressor failures raise uniform Python exceptions.
...
- Plot creation and key resolution: `chik/plotting/create_plots.py`, `chik/plotting/util.py`.
- Plot discovery/cache: `chik/plotting/manager.py`, `chik/plotting/cache.py`.
- Prover abstraction: `chik/plotting/prover.py`.
- Operator diagnostics: `chik/plotting/check_plots.py`.
- Consensus proof helpers: `chik/types/blockchain_format/proof_of_space.py`.
