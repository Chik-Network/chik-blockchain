...
Scope: `chik/simulator/`. This is distilled architectural context for future audit or implementation agents. It intentionally omits generated SSL collateral and obvious helper inventories.
...
`chik.simulator` is the deterministic test and local-dev control plane for Chik services. It does not implement an alternate blockchain. Instead, it fabricates blocks, plots, VDFs, keys, config roots, and RPC affordances so tests and simulator users can drive the real `FullNode`, `Blockchain`, `MempoolManager`, wallet sync, and service stack quickly.
...
`chik/_tests/util/setup_nodes.py` builds higher-level pytest environments on top of this module. `setup_simulators_and_wallets*()` creates one `BlockTools` per simulator and additional `BlockTools` if wallets outnumber simulators, then returns `FullNodeEnvironment`/`WalletEnvironment` wrappers plus the first `BlockTools`.
...
- `vdf_prover.get_vdf_info_and_proof()` calls `chikvdf.prove()` directly and wraps the output as `VDFInfo`/`VDFProof`. Simulator block creation depends on this being structurally identical to validation expectations, even with tiny test discriminants.
...
- Block generation and deterministic plot/key setup: `chik/simulator/block_tools.py`.
- Simulator full-node API and RPC routes: `chik/simulator/full_node_simulator.py`, `chik/simulator/simulator_full_node_rpc_api.py`, `chik/simulator/simulator_full_node_rpc_client.py`.
- Local simulator startup: `chik/simulator/start_simulator.py`.
- Test service factories: `chik/simulator/setup_services.py`, `chik/_tests/util/setup_nodes.py`.
- Minimal wallet spend helper: `chik/simulator/wallet_tools.py`.
