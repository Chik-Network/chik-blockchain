...
- `pyproject.toml` is the Python packaging authority: project metadata, Python bounds, dependencies/extras, dynamic versioning, supplemental Chik index, and console scripts.
- Console script names and targets are compatibility surfaces. Service-facing executables must stay aligned with `chik.util.service_groups`, CLI start/stop behavior, PyInstaller specs, installer scripts, and GUI expectations.
...
- `chik-blockchain-gui` is a git submodule, not part of the Python package, but install/build scripts bridge Python versioning, Node workspace layout, packaged daemon binaries, and Electron installers.
...
- Chik native dependencies (`chik_rs`, `chikpos`, `chikvdf`, `chikbip158`, `chik-puzzles-py`, `chiklisp`) define Rust/FFI and puzzle-bytecode boundaries used by consensus, wallet, plotting, and CLVK paths.
...
- `chik/__main__.py`, package startup glue, service launchers, daemon/RPC/config helpers, and service groups are coupled to root console scripts and packaged build entrypoints.

## Quality Gates

- `ruff.toml` selects broad lint coverage, enforces future annotations, bans relative imports, and bans direct `asyncio.create_task` in favor of `chik.util.task_referencer.create_referenced_task`.
...
- `.pre-commit-config.yaml` wires SQL checks, `__init__.py` generation, ruff format/check-fix, tach, Poetry checks, prettier/shfmt/basic hygiene, Chiklisp formatting, dependency-cycle checks, mypy config generation, and mypy.
...
- `test.yml` generates a dynamic test matrix through `chik/_tests/build-job-matrix.py`; scheduled/release/full-matrix runs broaden Python and platform coverage.
...
- `build_scripts/check_dependency_artifacts.py` protects release builders from unexpected source builds by checking artifacts from PyPI plus the Chik index.
- GUI install/build flows initialize/update the submodule, propagate `chik version` into GUI package metadata, and package Python/PyInstaller payloads into Electron artifacts.

Workflow changes should be treated as product changes to supported platforms. Check path filters, branch/tag triggers, concurrency behavior, generated matrix inputs, submodule assumptions, and Chik-managed GitHub action usage.
...
- Chain generation, full-sync replay, JSON block execution, CLVK/RPC tooling, dependency analysis, and benchmark helpers often encode external artifact formats or process behavior. Avoid cosmetic refactors that hide what contract a tool is exercising.
...
- GUI submodule integration: `chik-blockchain-gui`.
