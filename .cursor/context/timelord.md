...
- On Windows or with `slow_bluebox`, `chikvdf.prove()` runs in a `ThreadPoolExecutor`; a tempfile path is used as the shutdown trigger.
...
- `timelord_launcher.py` is a separate process manager for `chikvdf`'s `vdf_client` binary. It resolves the configured host, restarts clients until stopped, suppresses early stderr noise, and kills all active subprocesses on shutdown.
...
- Timelord service state and scheduling: `chik/timelord/timelord.py`, `chik/timelord/timelord_state.py`.
- Full-node-facing API and RPC metrics: `chik/timelord/timelord_api.py`, `chik/timelord/timelord_rpc_api.py`.
- Iteration derivation and VDF client launcher: `chik/timelord/iters_from_block.py`, `chik/timelord/timelord_launcher.py`.
- Service startup and local types: `chik/timelord/start_timelord.py`, `chik/timelord/types.py`.
