...
- `async_run_daemon()` runs `chik_init()`, initializes daemon logging, acquires
...
- Child processes inherit a copied environment with `CHIK_ROOT` set to the
...
- Plotting is a special daemon-managed pseudo-service named `chik_plotter`.
...
- Plotter command construction is per-plotter (`chikpos`, `bladebit`, `madmax`)
...
- `chik/_tests/core/daemon/test_daemon.py` covers daemon command responses,
...
- `chik/_tests/core/daemon/test_daemon_register.py` covers multi-service
  registration and connection cleanup semantics.
- `chik/_tests/core/daemon/test_keychain_proxy.py` covers local-vs-remote
  `KeychainProxy` behavior, private/public key reconstruction, and error mapping.
- `chik/_tests/core/test_daemon_rpc.py` is the minimal daemon client smoke test.
- `chik/_tests/cmds/test_daemon.py` anchors CLI daemon startup, keyring unlock
  flow, and daemon launcher behavior.

## Source Pointers

- Daemon websocket server and process control: `chik/daemon/server.py`.
- Client/proxy adapters: `chik/daemon/client.py`, `chik/daemon/keychain_proxy.py`.
- Remote keychain command handling: `chik/daemon/keychain_server.py`.
- Plotter process queue: `chik/daemon/server.py`, `chik/plotters/`.
- Message envelope helpers: `chik/util/ws_message.py`.
