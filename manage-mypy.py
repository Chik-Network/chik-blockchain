...
def build_exclusion_list(mypy_failures: list[str]) -> list[str]:
    ...
    #     # chik/cmds/wallet_funcs.py:1251: error: Incompatible types in assignment (expression has type "str", variable has type "int")  [assignment] # ruff: ignore[line-too-long]
...
def build_mypy_ini(check_exclusions: bool = False) -> None:
    ...
    mypy_config_data = (
        ...
        .replace("[mypy-chik-strict-bytes-exclusions]", strict_bytes_exclusion_section)
    )
