...
# installed chikvdf.
CHIKVDF_POETRY_INFO=$(.penv/bin/poetry show --no-ansi --no-interaction chikvdf)
echo "${CHIKVDF_POETRY_INFO}"
CHIKVDF_POETRY_INFO_VERSION=$(echo "${CHIKVDF_POETRY_INFO}" | grep 'version[[:space:]]*:' | sed 's/version[[:space:]]*: //')
echo "${CHIKVDF_POETRY_INFO_VERSION}"
CHIKVDF_VERSION="chikvdf==${CHIKVDF_POETRY_INFO_VERSION}"
echo "${CHIKVDF_VERSION}"
