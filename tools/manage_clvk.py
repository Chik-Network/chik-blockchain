...
root = here.parent
cache_path = root.joinpath(".chik_cache", "manage_clvk.json")
...
from chiklisp import compile_clvk  # ruff: ignore[module-import-not-at-top-of-file]

from chik.types.blockchain_format.serialized_program import (  # ruff: ignore[module-import-not-at-top-of-file]
    SerializedProgram,
)
...
clsp_suffix = ".clsp"
hex_suffix = ".clsp.hex"
all_suffixes = {"clsp": clsp_suffix, "hex": hex_suffix, "clvk": clvk_suffix}
...
class ManageClvkError(Exception):
    pass
...
class CacheVersionError(ManageClvkError):
    pass
...
class ClvkPaths:
    ...
    @classmethod
    def from_clvk(cls, clvk: pathlib.Path, hash_dict: dict[str, str] = {}) -> ClvkPaths:
        stem_filename = clvk.name[: -len(clsp_suffix)]
        hex_path = clvk.with_name(stem_filename + hex_suffix)
        missing_files = []
...
class ClvkBytes:
    ...
    @classmethod
    def from_clvk_paths(cls, paths: ClvkPaths, hash_dict: dict[str, str] = {}) -> ClvkBytes:
    ...
    @classmethod
    def from_hex_bytes(cls, hex_bytes: bytes) -> ClvkBytes:
        return cls(
            hex=hex_bytes,
...
def create_cache_entry(reference_paths: ClvkPaths, reference_bytes: ClvkBytes) -> CacheEntry:
    source_bytes = reference_paths.clvk.read_bytes()

    clvk_hasher = hashlib.sha256()
    clvk_hasher.update(source_bytes)
...
def check(use_cache: bool) -> int:
    ...
    print()
    print("Checking that no .clvk files begin with `(mod`")
    for stem_path in sorted(found_stems["clvk"]):
        with open(stem_path.with_name(stem_path.name + clvk_suffix)) as file:
            ...
            for line in file_lines:
                ...
                if "(" in non_comment:
                    ...
                    if len(non_comment) >= paren_index + 4 and non_comment[paren_index : paren_index + 4] == "(mod":
                        overall_fail = True
                        print(f"FAIL    : {stem_path.name + clvk_suffix} contains `(mod`")
                    break
    ...
    for stem_path in sorted(found_stems["clsp"]):
        ...
        try:
            reference_paths = ClvkPaths.from_clvk(clvk=clsp_path, hash_dict=HASHES)
            ...
            reference_bytes = ClvkBytes.from_clvk_paths(paths=reference_paths, hash_dict=HASHES)
            ...
            if not cache_hit:
                with tempfile.TemporaryDirectory() as temporary_directory:
                    generated_paths = ClvkPaths.from_clvk(
                        clvk=pathlib.Path(temporary_directory).joinpath(reference_paths.clvk.name),
                        hash_dict=HASHES,
                    )

                    compile_clvk(
                        input_path=os.fspath(reference_paths.clvk),
                        output_path=os.fspath(generated_paths.hex),
                        search_paths=[os.fspath(reference_paths.clvk.parent), str(std_libraries)],
                    )

                    generated_bytes = ClvkBytes.from_hex_bytes(hex_bytes=generated_paths.hex.read_bytes())
    ...
    if missing_files != []:
        overall_fail = True
        print()
        print("Missing files (run tools/manage_clvk.py build to build them):")
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temporary_directory:
                ...
                if reference_paths.hash not in new_hashes:
                    new_hashes[reference_paths.hash] = ClvkBytes.from_clvk_paths(
...
main(auto_envvar_prefix="CHIK_MANAGE_CLVK")
