...
from chik.wallet import wallet_rpc_client
from chik.wallet.wallet_request_types import Empty
from chik.wallet.wallet_rpc_metadata import WALLET_RPC_ENDPOINT_METADATA

REPO_ROOT = Path(__file__).resolve().parents[1]
OUTPUT_PATH = REPO_ROOT / "chik" / "wallet" / "wallet_rpc_client.pyi"
...
def generate() -> str:
    ...
    for meta in WALLET_RPC_ENDPOINT_METADATA:
        ...
        if response_type is Empty:
            response_ann = "None"
        elif _import_name(response_type) == "chik.wallet.wallet_request_types":
            response_ann = "wallet_request_types." + _type_name(response_type)
        else:
            response_ann = _type_name(response_type)

        if _import_name(request_type) == "chik.wallet.wallet_request_types":
    ...
    body = "\n".join(
        [
            ...
            "from chik.wallet import wallet_request_types",
